...
export function generateComprehensiveResponse(
  ...
  // ============================================================================
  if (
    lowerQuery.includes('medical history') ||
    ...
    return {
      ...
      detailed_summary: `**Medical History for ${p.name}:**\n\n**Chronic Conditions:**\n${mh.chronic_conditions
        .map(
          (c: any) =>
            `- **${c.condition}** (ICD-10: ${c.icd10})\n  Diagnosed: ${c.diagnosed_date}\n  Status: ${c.status}`
        )
        .join('\n')}\n\n**Past Surgeries:**\n${mh.past_surgeries.map((s: any) => `- **${s.procedure}** (${s.date})\n  Location: ${s.hospital}`).join('\n')}`,
    };
  }
