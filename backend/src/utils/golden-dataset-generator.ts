 ...
 */
export function generateSampleDataset(): GoldenDatasetEntry[] {
 ...
 */
function generateMedicationEntries(count: number): GoldenDatasetEntry[] {
  const medications = [
  ...
  for (let i = 0; i < count; i++) {
    ...
    entries.push({
      ...
      expected_intent: QueryIntent.RETRIEVE_MEDICATIONS,
      expected_entities: [
        {
      ...
      acceptance_criteria: CATEGORY_ACCEPTANCE_CRITERIA.medication,
...
function generateMedicationQuery(med: any, index: number): string {
  const queries = [
 ...
 */
function generateCarePlanEntries(count: number): GoldenDatasetEntry[] {
  ...
  for (let i = 0; i < count; i++) {
    ...
    entries.push({
      ...
      patient_id: 'user_3kmUMGZdobZMsMwxp8TFp4eSu1',
      expected_intent: QueryIntent.RETRIEVE_CARE_PLANS,
      expected_entities: [
        {
      ...
      ground_truth: {
        ...
        expected_extractions: [
          ...
          } as CarePlanExtraction,
        ],
        expected_confidence_min: 0.8,
        expected_sources_min: 2,
      },
      acceptance_criteria: CATEGORY_ACCEPTANCE_CRITERIA.care_plan,
      created_at: new Date().toISOString(),
...
function generateCarePlanQuery(condition: string, index: number): string {
  const queries = [
 ...
 */
function generateTemporalEntries(count: number): GoldenDatasetEntry[] {
  const entries: GoldenDatasetEntry[] = [];
  ...
  for (let i = 0; i < count; i++) {
    ...
    entries.push({
      ...
      patient_id: 'user_3kmUMGZdobZMsMwxp8TFp4eSu1',
      expected_intent: QueryIntent.RETRIEVE_RECENT,
      expected_entities: [],
      expected_temporal: {
      ...
      acceptance_criteria: CATEGORY_ACCEPTANCE_CRITERIA.temporal,
      created_at: new Date().toISOString(),
      verified_by: 'clinical_reviewer_1',
      metadata: {
        difficulty: i < 7 ? 'easy' : i < 14 ? 'medium' : 'hard',
...
function generateTemporalQuery(timeRef: string, index: number): string {
  const queries = [
...
function calculateDateFrom(relType: string, amount: number): string {
  const date = new Date();
  if (relType === 'weeks') {
 ...
 */
function generateEntityEntries(count: number): GoldenDatasetEntry[] {
  ...
  for (let i = 0; i < count; i++) {
    ...
    entries.push({
      ...
      patient_id: 'user_3kmUMGZdobZMsMwxp8TFp4eSu1',
      expected_intent: QueryIntent.SEARCH,
      expected_entities: [
        {
          text: entity,
      ...
      acceptance_criteria: CATEGORY_ACCEPTANCE_CRITERIA.entity,
      created_at: new Date().toISOString(),
      verified_by: 'clinical_reviewer_2',
      metadata: {
        difficulty: i < 5 ? 'easy' : i < 10 ? 'medium' : 'hard',
 ...
 */
function generateNegativeEntries(count: number): GoldenDatasetEntry[] {
  const entries: GoldenDatasetEntry[] = [];
  ...
  for (let i = 0; i < count; i++) {
    ...
    entries.push({
      ...
      acceptance_criteria: CATEGORY_ACCEPTANCE_CRITERIA.negative,
      created_at: new Date().toISOString(),
      verified_by: 'clinical_reviewer_1',
      metadata: {
        difficulty: 'medium',
 ...
 */
function generateAmbiguousEntries(count: number): GoldenDatasetEntry[] {
  const entries: GoldenDatasetEntry[] = [];
  ...
  for (let i = 0; i < count; i++) {
    ...
    entries.push({
      ...
      acceptance_criteria: CATEGORY_ACCEPTANCE_CRITERIA.ambiguous,
      created_at: new Date().toISOString(),
      verified_by: 'clinical_reviewer_2',
      metadata: {
        difficulty: 'hard',
