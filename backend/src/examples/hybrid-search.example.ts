 ...
 */
export async function exampleAddDocuments() {
  ...
  try {
    ...
    const clinicalNotes = [
      {
        ...
        chunk_text:
          'Patient diagnosed with Type 2 Diabetes mellitus. Started on Metformin 500mg twice daily. Blood glucose levels to be monitored weekly.',
        metadata: {
      ...
      },
      {
        ...
        chunk_text:
          'Hemoglobin A1C: 7.2% (elevated, target <7%). Fasting glucose: 145 mg/dL. Patient adherence to medication regimen is good.',
        metadata: {
      ...
      },
      {
        ...
        chunk_text:
          'Blood pressure reading: 140/90 mmHg (stage 1 hypertension). Recommended lifestyle modifications including reduced sodium intake.',
        metadata: {
      ...
      },
      {
        ...
        chunk_text:
          'Increased Metformin dosage to 1000mg twice daily due to suboptimal glucose control. Continue monitoring blood sugar levels.',
        metadata: {
      ...
      },
      {
        ...
        chunk_text:
          'Patient reports improved energy levels and no hypoglycemic episodes. Blood glucose readings averaging 120 mg/dL over past two weeks.',
        metadata: {
    ...
    const documents: Document[] = clinicalNotes.map((note, i) => ({
      ...note,
...
export async function exampleSemanticSearch() {
  ...
  try {
    ...
    const results = await hybridSearchEngine.search(query, queryEmbedding, options);
 ...
 */
export async function exampleKeywordSearch() {
 ...
 */
export async function exampleBalancedSearch() {
 ...
 */
export async function exampleFilteredSearch() {
 ...
 */
export async function exampleRecencyBoost() {
  ...
  try {
    ...
    const optionsNoBoost: SearchOptions = {
      k: 5,
      alpha: 0.6,
    ...
    const resultsNoBoost = await hybridSearchEngine.search(
      query,
      queryEmbedding,
      optionsNoBoost
    );
    ...
    const optionsWithBoost: SearchOptions = {
      k: 5,
      alpha: 0.6,
      filters: { patientId: 'patient_123' },
      recencyBoost: true,
    ...
    const resultsWithBoost = await hybridSearchEngine.search(
      query,
      queryEmbedding,
      optionsWithBoost
    );
    ...
    resultsWithBoost.forEach((result, i) => {
      console.log(
        `    ${i + 1}. Score: ${result.score.toFixed(4)}, Boost: ${result.recencyBoost.toFixed(3)}, Date: ${result.metadata.occurred_at}`
      );
    });
 ...
 */
export async function exampleAlphaComparison() {
 ...
 */
export async function exampleRAGPipeline() {
