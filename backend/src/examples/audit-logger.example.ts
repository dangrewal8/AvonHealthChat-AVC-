...
import auditLogger, { QueryRequest } from '../services/audit-logger.service';
import { UIResponse, QueryMetrics } from '../services/ui-response-builder.service';
 ...
 */
function generateSampleRequest(patientId: string, queryText: string): QueryRequest {
  return {
 ...
 */
function generateSampleResponse(queryId: string, patientId: string): UIResponse {
 ...
 */
export async function exampleLogSuccessfulQuery() {
  ...
  await auditLogger.logQuery(request, response, metrics);
...
export async function exampleLogFailedQuery() {
  ...
  await auditLogger.logQuery(request, null, metrics, error);
 ...
 */
export async function exampleGetQueryHistory() {
  ...
  // Get history
  const history = await auditLogger.getQueryHistory(patientId, 10);
 ...
 */
export async function exampleSearchByText() {
  ...
  const results = await auditLogger.searchQueries({
...
export async function exampleFilterByDateRange() {
  ...
  const results = await auditLogger.filterByDateRange(
 ...
 */
export async function exampleFilterByConfidence() {
  console.log('Example 6: Filter by Confidence Score');
  console.log('-'.repeat(80));

  // Search for high-confidence queries (> 0.8)
  const highConfidence = await auditLogger.searchQueries({
    min_confidence: 0.8,
  });
  ...
  const lowConfidence = await auditLogger.searchQueries({
    max_confidence: 0.5,
  });
 ...
 */
export async function exampleRetentionPolicy() {
  console.log('Example 11: Retention Policy');
  console.log('-'.repeat(80));

  // Set retention policy
  auditLogger.setRetentionPolicy({
    retention_days: 90,
    auto_cleanup: true,
...
export async function exampleLogFilePath() {
  ...
  console.log(`    Total entries: ${auditLogger.getTotalCount()}\n`);
...
export async function exampleExplain() {
  ...
  const explanation = auditLogger.explain();
