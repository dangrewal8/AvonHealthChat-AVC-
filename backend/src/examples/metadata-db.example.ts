...
import metadataDB, { Chunk } from '../services/metadata-db.service';
 ...
 */
export async function exampleInsertChunks() {
...
export async function exampleFilterByDateRange() {
  ...
  try {
    ...
    const chunkIds = await metadataDB.filterByDateRange(patientId, fromDate, toDate);
...
export async function exampleFilterByType() {
  ...
  try {
    ...
    const chunkIds = await metadataDB.filterByType(patientId, types);
...
export async function exampleCombinedFiltering() {
  ...
  try {
    const chunkIds = await metadataDB.filterChunks({
 ...
 */
export async function exampleGetChunkById() {
 ...
 */
export async function exampleGetChunksByIds() {
...
export async function exampleGetStats() {
  ...
  try {
    const stats = await metadataDB.getStats();
...
export async function exampleRAGPipeline() {
  ...
  try {
    ...
    const filteredChunkIds = await metadataDB.filterChunks({
      patientId,
      fromDate: '2024-01-01T00:00:00Z',
    ...
    const chunks = await metadataDB.getChunksByIds(topChunkIds);
 ...
 */
export async function exampleDeleteChunks() {
  console.log('Example 10: Delete Chunks');
  console.log('-'.repeat(80));

  try {
    const chunkIds = ['chunk_001', 'chunk_002'];
    const deletedCount = await metadataDB.deleteChunks(chunkIds);
