...
import cacheManager, { RetrievalResult } from '../services/cache-manager.service';
 ...
 */
export async function exampleEmbeddingCache() {
 ...
 */
export async function exampleQueryResultsCache() {
  ...
  try {
    ...
    cacheManager.cacheQueryResult(query, patientId, filters, mockResult);

    // Retrieve from cache
    console.log('  [Second Query] Retrieving from cache');
    const cached = cacheManager.getQueryResult(query, patientId, filters);
    ...
    const cached2 = cacheManager.getQueryResult(query, patientId, differentFilters);
    ...
    console.log('  Cache Statistics:');
    console.log(
      `    Query results cache size: ${stats.queryResults.size}/${stats.queryResults.maxSize}`
    );
 ...
 */
export async function examplePatientIndexCache() {
  ...
  try {
    ...
    for (const patientId of patientIds) {
      ...
      const index = await cacheManager.loadPatientIndex(patientId);
    ...
    for (const patientId of patientIds) {
      const start = Date.now();
      const index = cacheManager.getPatientIndex(patientId);
      const duration = Date.now() - start;
    ...
    console.log('  Cache Statistics:');
    console.log(
      `    Patient index cache size: ${stats.patientIndices.size}/${stats.patientIndices.maxSize}`
    );
 ...
 */
export async function exampleLRUEviction() {
  ...
  try {
    ...
    for (let i = 0; i < patients.length; i++) {
      ...
      try {
        await cacheManager.loadPatientIndex(patients[i]);
      } catch (error) {
    ...
    const evicted = cacheManager.getPatientIndex('patient_001');
 ...
 */
export async function exampleCacheKeyGeneration() {
  ...
  try {
    ...
    for (let i = 0; i < variants.length; i++) {
      ...
      const start = Date.now();
      await embeddingService.generateEmbedding(variants[i]);
      const duration = Date.now() - start;
...
export async function exampleCacheStatistics() {
  ...
  try {
    ...
    console.log(
      `    Size: ${stats.queryResults.size}/${stats.queryResults.maxSize} entries`
    );
    ...
    console.log(
      `    Size: ${stats.patientIndices.size}/${stats.patientIndices.maxSize} entries`
    );
...
export async function exampleCacheInvalidation() {
  ...
  try {
    ...
    await cacheManager.loadPatientIndex(patientId);
    console.log('    ✓ Loaded and cached\n');

    // Verify it's cached
    const cached1 = cacheManager.getPatientIndex(patientId);
    ...
    cacheManager.invalidatePatientIndex(patientId);
    console.log('    ✓ Cache invalidated\n');

    // Verify it's no longer cached
    const cached2 = cacheManager.getPatientIndex(patientId);
