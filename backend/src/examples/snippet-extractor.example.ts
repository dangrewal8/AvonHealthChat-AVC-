...
import snippetExtractor from '../services/snippet-extractor.service';
...
bleeding or stomach upset. Follow up in 2 weeks if pain persists.
  ...
  const charOffsets: [number, number] = [27, 51]; // "ibuprofen 400 mg q6h PRN"

  const snippet = snippetExtractor.extract(text, charOffsets);
 ...
 */
export function exampleSentenceBoundaries() {
...
No adverse effects noted. Continue current regimen.
  ...
  const charOffsets: [number, number] = [38, 58]; // "ibuprofen 400 mg"

  // With sentence boundaries (default)
  const withSentences = snippetExtractor.extract(text, charOffsets, {
    preferSentences: true,
  });

  // Without sentence boundaries
  const withoutSentences = snippetExtractor.extract(text, charOffsets, {
 ...
 */
export function exampleMaxLength() {
...
shows HbA1c of 7.2%, creatinine 1.4 mg/dL, and LDL cholesterol 95 mg/dL.
  ...
  const charOffsets: [number, number] = [159, 187];

  const snippet = snippetExtractor.extract(longText, charOffsets);
 ...
 */
export function exampleEllipsis() {
...
for pain management. This is the end of the document.
  ...
  const charOffsets: [number, number] = [66, 82]; // "ibuprofen 400 mg"

  // With ellipsis (default)
  const withEllipsis = snippetExtractor.extract(text, charOffsets, {
    addEllipsis: true,
  });

  // Without ellipsis
  const withoutEllipsis = snippetExtractor.extract(text, charOffsets, {
    addEllipsis: false,
  });
...
should be taken with food to minimize gastric irritation.
  ...
  const markdown = snippetExtractor.extractWithHighlight(text, charOffsets);

  // HTML highlight
  const html = snippetExtractor.extractWithHighlight(
    text,
    charOffsets,
    {},
    ['<mark>', '</mark>']
 ...
 */
export function exampleMultipleSnippets() {
...
atorvastatin 40 mg daily. All medications should be taken as prescribed.
  ...
  const offsetsList: [number, number][] = [
    [21, 41], // "lisinopril 10 mg daily"
    [43, 71], // "metformin 1000 mg twice daily"
    [73, 97], // "atorvastatin 40 mg daily"
  ];

  const snippets = snippetExtractor.extractMultiple(text, offsetsList, {
    contextChars: 30,
  });
...
should be taken with food to minimize gastric irritation.
  ...
  const charOffsets: [number, number] = [27, 51];

  // Minimal context (20 chars)
  const minimal = snippetExtractor.extract(text, charOffsets, {
    contextChars: 20,
    maxLength: 100,
    preferSentences: false,
    addEllipsis: true,
  ...
  const generous = snippetExtractor.extract(text, charOffsets, {
    contextChars: 80,
    maxLength: 250,
    preferSentences: true,
    addEllipsis: true,
 ...
 */
export function exampleSnippetInfo() {
...
should be taken with food to minimize gastric irritation.
  ...
  const info = snippetExtractor.getSnippetInfo(text, charOffsets);
 ...
 */
export function exampleDocumentStart() {
...
Additional instructions follow.
  ...
  const charOffsets: [number, number] = [1, 25]; // "Ibuprofen 400 mg q6h PRN"
 ...
 */
export function exampleDocumentEnd() {
...
Follow up in 2 weeks. Continue current regimen including ibuprofen 400 mg PRN.
  ...
  const charOffsets: [number, number] = [59, 79]; // "ibuprofen 400 mg PRN"
 ...
 */
export function exampleShortText() {
  console.log('Example 11: Short Text (No Truncation)');
  console.log('-'.repeat(80));

  const text = 'Prescribed ibuprofen 400 mg.';

  const charOffsets: [number, number] = [11, 27]; // "ibuprofen 400 mg"
...
previous NSAID use. No contraindications noted. Will monitor for GI side effects.
  ...
  const provenance = {
    artifact_id: 'note_456',
    char_offsets: [82, 106] as [number, number], // "ibuprofen 400 mg q6h PRN"
  };

  // Extract snippet for UI display (per ChatGPT spec)
  const snippet = snippetExtractor.extract(
    chunk.chunk_text,
  ...
  console.log(
    `    "${chunk.chunk_text.substring(
      provenance.char_offsets[0],
      provenance.char_offsets[1]
    )}"\n`
  );
...
The patient was prescribed ibuprofen 400 mg q6h PRN for pain. Monitor for side effects.
  ...
  const isValid = snippetExtractor.validateSnippet(text, charOffsets);
 ...
 */
export function exampleMedicalRecord() {
...
2. Hypertension - well controlled
  ...
  const charOffsets: [number, number] = [132, 165]; // "ibuprofen 400 mg PO q6h PRN"

  const snippet = snippetExtractor.extract(record, charOffsets);
  const highlighted = snippetExtractor.extractWithHighlight(record, charOffsets);
...
export function exampleExplain() {
  ...
  const explanation = snippetExtractor.explain();
