...
import retrievalScorer, { ScoringWeights } from '../services/retrieval-scorer.service';
  ...
  return [
    {
      ...
      content:
        'Patient prescribed metformin 500mg twice daily for type 2 diabetes management. Good glycemic control.',
      metadata: {
    ...
    },
    {
      ...
      patient_id: 'patient_123',
      content:
        'Blood pressure measured at 120/80 mmHg. Patient continues on metformin. No side effects reported.',
      metadata: {
...
export function exampleBasicScoring() {
  ...
  chunks.forEach((chunk, i) => {
    const candidate = retrievalScorer.scoreCandidate(chunk, query, similarities[i]);
 ...
 */
export function exampleKeywordMatching() {
  ...
  queries.forEach((queryText, i) => {
    ...
    chunks.forEach((chunk) => {
      const score = retrievalScorer.calculateKeywordMatch(chunk.content, queryText);
 ...
 */
export function exampleRecencyBoost() {
  ...
  chunks.forEach((chunk) => {
    const recencyBoost = retrievalScorer.calculateRecencyBoost(chunk.metadata.date);
 ...
 */
export function exampleTypePreference() {
  ...
  intents.forEach((intent) => {
    ...
    chunks.forEach((chunk) => {
      const typeScore = retrievalScorer.calculateTypePreference(
        chunk.metadata.artifact_type,
        intent
      );

      console.log(
        `    ${chunk.metadata.artifact_type.padEnd(20)} → ${typeScore.toFixed(1)}`
      );
    });
 ...
 */
export function exampleScoreCombination() {
  ...
  const weights = retrievalScorer.getDefaultWeights();
  ...
  scores.forEach((score, i) => {
    const combined = retrievalScorer.combineScores(
      score.semantic,
      score.keyword,
      score.recency,
      score.type
...
export function exampleRanking() {
  ...
  const ranked = retrievalScorer.scoreAndRank(chunks, query, similarities);
...
export function exampleNormalization() {
  ...
  const candidates = chunks.map((chunk, i) =>
    retrievalScorer.scoreCandidate(chunk, query, similarities[i])
  );
  ...
  const normalized = retrievalScorer.normalizeScores(candidates);
...
export function exampleCustomWeights() {
  ...
  const ranked1 = retrievalScorer.scoreAndRank(chunks, query, similarities, 3);
  ranked1.forEach((c) => {
    console.log(`    ${c.rank}. ${c.chunk.chunk_id} - Score: ${c.scores.combined.toFixed(3)}`);
  });

  // Re-rank with recency emphasis
  const reranked = retrievalScorer.rerank(ranked1, recencyWeights);
  ...
  const keywordWeights: ScoringWeights = {
    semantic: 0.3,
  ...
  const ranked2 = retrievalScorer.rerank(ranked1, keywordWeights);
  ranked2.forEach((c) => {
    console.log(`    ${c.rank}. ${c.chunk.chunk_id} - Score: ${c.scores.combined.toFixed(3)}`);
  });
 ...
 */
export function exampleScoreExplanation() {
  ...
  const ranked = retrievalScorer.scoreAndRank(chunks, query, similarities, 2);

  console.log('  Detailed explanations for top 2 candidates:\n');

  ranked.forEach((candidate, i) => {
    console.log(`  === Candidate ${i + 1} ===`);
    console.log(retrievalScorer.explainRanking(candidate));
    console.log('');
  });
 ...
 */
export function exampleScoreBreakdown() {
  ...
  candidates.forEach((candidate, i) => {
    const breakdown = retrievalScorer.getScoreBreakdown(candidate);
 ...
 */
export function exampleDiversityRanking() {
  ...
  const diversityRanked = retrievalScorer.diversityRank(ranked);
...
export function exampleTopK() {
  ...
  topKValues.forEach((k) => {
    const topK = retrievalScorer.scoreAndRank(chunks, query, similarities, k);
 ...
 */
export function exampleBatchScoring() {
  ...
  const candidates = retrievalScorer.batchScore(chunks, query, similarities);
...
export function exampleFullPipeline() {
  ...
  const ranked = retrievalScorer.rankCandidates(candidates, 3);
