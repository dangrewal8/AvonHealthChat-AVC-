...
import followUpResolver, {
  QueryIntent,
  StructuredQuery,
  ConversationContext,
} from '../services/follow-up-resolver.service';
...
export function exampleBasicDetection() {
  ...
  queries.forEach(query => {
    const isFollowUp = followUpResolver.isFollowUp(query);
    console.log(`    "${query}"`);
 ...
 */
export function exampleContextMerging() {
  console.log('Example 2: Context Merging');
  console.log('-'.repeat(80));

  // Previous query
  const previousQuery: StructuredQuery = {
  ...
  // Merge context
  const merged = followUpResolver.mergeContext(newQuery, previousQuery);
 ...
 */
export function exampleTemporalInheritance() {
  ...
  const previousQuery: StructuredQuery = {
    ...
    patient_id: 'patient_123',
    intent: QueryIntent.RETRIEVE_LAB_RESULTS,
  ...
  const scenarios = [
    ...
    { query: 'And this week?', temporal: { type: 'relative' as const, relative: 'this_week' as const }, description: 'New temporal → override' },
 ...
 */
export function exampleIntentPreservation() {
  ...
  const followUps = [
    { query: 'What about last month?', intent: QueryIntent.UNKNOWN, description: 'Preserve' },
    { query: 'And blood pressure?', intent: QueryIntent.UNKNOWN, description: 'Preserve' },
    { query: 'Show me lab results', intent: QueryIntent.RETRIEVE_LAB_RESULTS, description: 'Override' },
  ];
 ...
 */
export function exampleFollowUpResolution() {
  ...
  const context: ConversationContext = {
    ...
    last_intent: QueryIntent.RETRIEVE_MEDICATIONS,
  };
  ...
  const isFollowUp = followUpResolver.isFollowUp(followUpQuery);

  console.log('    Detected as follow-up:', isFollowUp);

  if (isFollowUp) {
    const resolved = followUpResolver.resolveFollowUp(followUpQuery, context);
 ...
 */
export function exampleFollowUpChain() {
  ...
  queries.forEach((item, i) => {
    const isFollowUp = followUpResolver.isFollowUp(item.query);
    ...
    if (isFollowUp && currentContext.last_query) {
      const resolved = followUpResolver.resolveFollowUp(item.query, currentContext);
 ...
 */
export function exampleFollowUpPatterns() {
  console.log('Example 8: Different Follow-Up Patterns');
  console.log('-'.repeat(80));

  const patterns = followUpResolver.getFollowUpPatterns();
  ...
  examples.forEach(example => {
    const isFollowUp = followUpResolver.isFollowUp(example.query);
 ...
 */
export function exampleMedicationConversation() {
  ...
  conversation.forEach((turn, i) => {
    ...
    if (turn.role === 'Patient' && turn.query) {
      const isFollowUp = followUpResolver.isFollowUp(turn.query);

      if (isFollowUp && context.last_query) {
        const resolved = followUpResolver.resolveFollowUp(turn.query, context);
 ...
 */
export function exampleLabResultsConversation() {
  ...
  queries.forEach((query, i) => {
    ...
    if (isFollowUp && context.last_query) {
      const resolved = followUpResolver.resolveFollowUp(query, context);
      console.log('    Resolved:');
      console.log('      Intent:', resolved.intent);
      console.log('      Entities:', resolved.entities.map(e => e.value).join(', ') || 'none');
      console.log('      Temporal:', resolved.temporal_filter?.relative || 'none');
...
export function exampleEdgeCases() {
  ...
  const followUp1 = followUpResolver.resolveFollowUp('What about last week?', context1);
  ...
  const isShortFollowUp = followUpResolver.isFollowUp(shortQuery);
  ...
  const isLongFollowUp = followUpResolver.isFollowUp(longQuery);
...
export function exampleExplain() {
  ...
  const explanation = followUpResolver.explain();
