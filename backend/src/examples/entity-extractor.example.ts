 ...
 */
export function exampleMedicationExtraction() {
  ...
  queries.forEach((query, i) => {
    ...
    const medications = entityExtractor.extractByType(query, 'medication');
 ...
 */
export function exampleConditionExtraction() {
  ...
  queries.forEach((query, i) => {
    ...
    const conditions = entityExtractor.extractByType(query, 'condition');
 ...
 */
export function exampleSymptomExtraction() {
  ...
  queries.forEach((query, i) => {
    ...
    const symptoms = entityExtractor.extractByType(query, 'symptom');
 ...
 */
export function exampleDateExtraction() {
  ...
  queries.forEach((query, i) => {
    ...
    const dates = entityExtractor.extractByType(query, 'date');
 ...
 */
export function examplePersonExtraction() {
  ...
  queries.forEach((query, i) => {
    ...
    const persons = entityExtractor.extractByType(query, 'person');
 ...
 */
export function exampleMixedEntityExtraction() {
  ...
  queries.forEach((query, i) => {
    ...
    if (entities.length > 0) {
      ...
      entities.forEach((entity) => {
        ...
        if (entity.position) {
          console.log(
            `        Position: ${entity.position.start}-${entity.position.end}`
          );
        }
        console.log('');
      });
 ...
 */
export function exampleEntityNormalization() {
...
export function exampleAbbreviationExpansion() {
  ...
  const abbreviations = entityExtractor.getAbbreviations();
 ...
 */
export function exampleConfidenceScoring() {
  console.log('Example 9: Confidence Scoring');
  console.log('-'.repeat(80));

  const query =
    'Patient with diabetes experiencing chest pain, taking metformin 500mg BID, saw Dr. Smith yesterday';
 ...
 */
export function exampleEntityTypeChecking() {
  ...
  queries.forEach((query, i) => {
    ...
    const hasMedication = entityExtractor.hasEntityType(query, 'medication');
    const hasCondition = entityExtractor.hasEntityType(query, 'condition');
    const hasSymptom = entityExtractor.hasEntityType(query, 'symptom');
    const hasDate = entityExtractor.hasEntityType(query, 'date');
 ...
 */
export function exampleEntityPatternInspection() {
  console.log('Example 11: Entity Pattern Inspection');
  console.log('-'.repeat(80));

  console.log('  Inspecting entity patterns by type:\n');

  const types: Array<'medication' | 'condition' | 'symptom' | 'person'> = [
    'medication',
    'condition',
    'symptom',
    'person',
  ...
  types.forEach((type) => {
    const patterns = entityExtractor.getEntityPatterns(type);
 ...
 */
export function exampleComplexMedicalQuery() {
  console.log('Example 12: Complex Medical Query');
  console.log('-'.repeat(80));

  const query =
    'Patient with type 2 diabetes and hypertension on metformin 1000mg BID and lisinopril 10mg QD. ' +
