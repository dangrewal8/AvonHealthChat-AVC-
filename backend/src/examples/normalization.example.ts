 ...
 */
export function exampleNormalizeCarePlan() {
  const rawCarePlan = {
    id: 'cp_123',
  ...
  const artifact = normalizationService.normalizeCarePlan(rawCarePlan);
 ...
 */
export function exampleNormalizeMedication() {
  const rawMedication = {
    id: 'med_789',
    patient_id: 'patient_456',
    name: 'Metformin',
  ...
  const artifact = normalizationService.normalizeMedication(rawMedication);
 ...
 */
export function exampleNormalizeNote() {
  const rawNote = {
    id: 'note_321',
    patient_id: 'patient_456',
  ...
  const artifact = normalizationService.normalizeNote(rawNote);
 ...
 */
export function exampleMissingFields() {
  const rawData = {
    // Missing id, patient_id, date
    description: 'Some medical note',
  };

  const artifact = normalizationService.normalizeNote(rawData);
 ...
 */
export function exampleDateConversion() {
  const testDates = [
  ...
  testDates.forEach((rawData, index) => {
    const artifact = normalizationService.normalizeNote({
      id: `note_${index}`,
 ...
 */
export function exampleNestedText() {
  const rawData = {
    id: 'note_999',
    patient_id: 'patient_123',
    content: {
...
export function exampleValidation() {
  ...
  const result = normalizationService.normalize(invalidData, 'note');
 ...
 */
export function exampleBatchNormalization() {
  const rawCarePlans = [
    {
      id: 'cp_1',
      patient_id: 'patient_123',
  ...
  const normalized = normalizationService.normalizeBatch(rawCarePlans, 'care_plan');
 ...
 */
export function exampleTextExtraction() {
  const testCases = [
 ...
 */
export async function exampleIntegrationWithEMR() {
  // This would typically be used within the EMR service
  const rawCarePlans: any[] = [
    /* fetched from API */
  ];

  // Normalize all fetched care plans
  const normalizedCarePlans = normalizationService.normalizeBatch(rawCarePlans, 'care_plan');
 ...
 */
export function exampleMedicationNormalization() {
  const medications = [
    {
      id: 'med_1',
      patient_id: 'patient_123',
  ...
  const normalized = medications.map((med) => normalizationService.normalizeMedication(med));
 ...
 */
export function exampleSourceUrls() {
  const artifacts = [
    normalizationService.normalizeCarePlan({
      id: 'cp_123',
      patient_id: 'p1',
      description: 'Plan',
      created_at: '2024-01-01T10:00:00Z',
    }),
    normalizationService.normalizeMedication({
      id: 'med_456',
      patient_id: 'p1',
      name: 'Drug',
      prescribed_at: '2024-01-01T10:00:00Z',
    }),
    normalizationService.normalizeNote({
      id: 'note_789',
      patient_id: 'p1',
      content: 'Note',
      created_at: '2024-01-01T10:00:00Z',
