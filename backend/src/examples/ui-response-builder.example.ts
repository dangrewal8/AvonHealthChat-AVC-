...
import uiResponseBuilder, { QueryMetrics } from '../services/ui-response-builder.service';
import { GeneratedAnswer } from '../services/answer-generation-agent.service';
import { FormattedProvenance } from '../services/provenance-formatter.service';
import { ConfidenceScore } from '../services/confidence-scorer.service';
 ...
 */
function generateSampleAnswer(): GeneratedAnswer {
  ...
  return {
    ...
    detailed_summary:
      'Patient is currently taking Metformin 500mg twice daily for Type 2 Diabetes management (note_123). Blood glucose levels are improving with the current regimen.',
 ...
 */
function generateSampleConfidence(): ConfidenceScore {
  return {
    score: 0.85,
    label: 'high',
    components: {
 ...
 */
function generateSampleMetrics(): QueryMetrics {
 ...
 */
export function exampleBuildResponse() {
  ...
  const response = uiResponseBuilder.build(answer, provenance, confidence, query, metrics);

  // Update sources count
  uiResponseBuilder.updateSourcesCount(response, provenance.length);
 ...
 */
export function exampleFormatMetadata() {
  ...
  const metadata = uiResponseBuilder.formatMetadata(query, metrics, 'meditron');
 ...
 */
export function exampleBuildErrorResponse() {
  console.log('Example 3: Build Error Response');
  console.log('-'.repeat(80));

  const queryId = 'query_error_123';
  const errorResponse = uiResponseBuilder.buildErrorResponse(
    queryId,
    'RETRIEVAL_FAILED',
    'Failed to retrieve candidates from vector store',
    { reason: 'Connection timeout', retry: true }
  );
 ...
 */
export function exampleValidateResponse() {
  ...
  try {
    uiResponseBuilder.build(answer, provenance, confidence, query, metrics);
  ...
  try {
    ...
    uiResponseBuilder.validateResponse(invalidResponse);
 ...
 */
export function exampleAddUserInfo() {
  ...
  uiResponseBuilder.addUserInfo(response, 'user_789', 'session_abc123');
...
export function exampleToJSON() {
  ...
  const compactJSON = uiResponseBuilder.toJSON(response, false);
  console.log(`    ${compactJSON.substring(0, 100)}...\n`);

  console.log('  Pretty JSON (first 200 chars):');
  const prettyJSON = uiResponseBuilder.toJSON(response, true);
 ...
 */
export function exampleGetResponseSize() {
  ...
  const sizeBytes = uiResponseBuilder.getResponseSize(response);
  const sizeKB = (sizeBytes / 1024).toFixed(2);

  console.log('  Response Size:\n');
  console.log(`    Bytes: ${sizeBytes}`);
  console.log(`    KB: ${sizeKB}`);
  console.log(`    Too large (> 1MB): ${uiResponseBuilder.isResponseTooLarge(response) ? 'Yes' : 'No'}\n`);
 ...
 */
export function exampleTruncateResponse() {
  ...
  const truncated = uiResponseBuilder.truncateResponse(response, 1000);
 ...
 */
export function exampleGetResponseSummary() {
  ...
  const summary = uiResponseBuilder.getResponseSummary(response);
 ...
 */
export function exampleAuditInformation() {
 ...
 */
export function exampleCompletePipeline() {
  ...
  const response = uiResponseBuilder.build(answer, provenance, confidence, query, metrics);
  uiResponseBuilder.updateSourcesCount(response, provenance.length);
  uiResponseBuilder.addUserInfo(response, 'user_123', 'session_456');
...
export function exampleExplain() {
  ...
  const explanation = uiResponseBuilder.explain();
