...
import retryManager from '../services/retry-manager.service';

/**
 * Example 1: Basic retry
 */
export async function exampleBasicRetry() {
 ...
 */
export async function exampleExponentialBackoff() {
  console.log('Example 2: Exponential Backoff');
  console.log('-'.repeat(80));

  const delays = [];
  for (let i = 0; i < 5; i++) {
    const delay = retryManager.getBackoffDelay(i);
    delays.push(delay);
  }
 ...
 */
export async function exampleRetryableErrors() {
 ...
 */
export async function exampleCircuitBreakerClosed() {
  ...
  console.log('  Initial state:', retryManager.getCircuitState());
  ...
  try {
    ...
    console.log('  After success:', retryManager.getCircuitState());
 ...
 */
export async function exampleCircuitBreakerOpen() {
  ...
  console.log('\n  Circuit state:', retryManager.getCircuitState());
 ...
 */
export async function exampleFallbackRetrievalOnly() {
  ...
  const fallbackResponse = retryManager.fallbackRetrievalOnly(retrievalResults);
 ...
 */
export async function exampleFallbackKeywordSearch() {
  console.log('Example 8: Fallback - Keyword Search');
  console.log('-'.repeat(80));

  const query = 'What medications is the patient taking?';
  const fallbackResults = retryManager.fallbackKeywordSearch(query);
 ...
 */
export async function exampleFallbackSuggestRefinement() {
  console.log('Example 9: Fallback - Suggest Refinement');
  console.log('-'.repeat(80));

  const query = 'xyz123 test query';
  const fallbackResponse = retryManager.fallbackSuggestRefinement(query);
...
export async function exampleWithFallback() {
  ...
  const result = await retryManager.withFallback(
    primaryFn,
    fallbackFn,
    'RETURN_CACHED'
  );
 ...
 */
export async function exampleNonRetryableError() {
 ...
 */
export async function exampleAsyncIntegration() {
  ...
  try {
    const result = await retryManager.retry(unreliableAsyncOp);
 ...
 */
export async function exampleRealWorldLLM() {
  ...
  try {
    const result = await retryManager.retry(generateWithLLM, {
      maxAttempts: 3,
      backoffMs: 2000,
    });
...
export function exampleExplain() {
  ...
  const explanation = retryManager.explain();
