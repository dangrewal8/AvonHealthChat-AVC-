...
import resultDiversifier from '../services/result-diversifier.service';
 ...
 */
export function exampleBasicDiversification() {
  ...
  const diversified = resultDiversifier.diversify(candidates);
 ...
 */
export function examplePenaltyCalculation() {
  ...
  positions.forEach((pos) => {
    const penalty = resultDiversifier.calculateDiversityPenalty(pos);
    const reduction = (1 - penalty) * 100;

    let interpretation = '';
    if (pos === 1) interpretation = 'No penalty';
    else if (pos === 2) interpretation = '10% penalty';
    else if (pos === 3) interpretation = '19% penalty';
    else if (pos === 4) interpretation = '27% penalty';
    else interpretation = `${reduction.toFixed(0)}% penalty`;

    console.log(
      `  ${String(pos).padStart(8)} │ ${penalty.toFixed(4).padStart(7)} │ ${reduction.toFixed(1).padStart(9)}% │ ${interpretation}`
    );
  });
 ...
 */
export function exampleGrouping() {
  console.log('Example 3: Grouping by Artifact');
  console.log('-'.repeat(80));

  const candidates = generateSampleCandidates();

  const grouped = resultDiversifier.groupByArtifact(candidates);
 ...
 */
export function exampleDiversityStats() {
  console.log('Example 4: Diversity Statistics');
  console.log('-'.repeat(80));

  const candidates = generateSampleCandidates();

  const statsBefore = resultDiversifier.calculateDiversityStats(candidates);
  ...
  const statsAfter = resultDiversifier.calculateDiversityStats(diversified);
...
export function exampleBeforeAfterComparison() {
  ...
  const comparison = resultDiversifier.compareBeforeAfter(candidates, diversified);
...
export function exampleMinimumDiversity() {
  ...
  const diversified = resultDiversifier.diversify(biasedCandidates);
  const ensured = resultDiversifier.ensureMinimumDiversity(diversified, 5, 2);
 ...
 */
export function examplePenaltyCurve() {
  console.log('Example 7: Penalty Curve Visualization');
  console.log('-'.repeat(80));

  const curve = resultDiversifier.getPenaltyCurve(10);
  ...
  curve.forEach((point) => {
    ...
    console.log(
      `    Pos ${point.position.toString().padStart(2)}: ${bar.padEnd(50)} ${point.penalty.toFixed(3)} (-${point.penalty_pct.toFixed(0)}%)`
    );
  });
 ...
 */
export function exampleExplainDiversification() {
  ...
  if (penalizedCandidate) {
    console.log('\n' + resultDiversifier.explainDiversification(penalizedCandidate) + '\n');
  }
 ...
 */
export function exampleMostPenalized() {
  ...
  const mostPenalized = resultDiversifier.findMostPenalized(diversified, 20);
 ...
 */
export function exampleCheckDiversity() {
  ...
  const hasDiversity = resultDiversifier.hasMinimumDiversity(diversified, 5, 2);
 ...
 */
export function exampleBatchDiversification() {
  ...
  const diversifiedLists = resultDiversifier.batchDiversify(candidateLists);

  diversifiedLists.forEach((diversified, i) => {
    const avgPenalty =
      diversified.reduce((sum, c) => sum + c.diversity_penalty, 0) / diversified.length;

    const stats = resultDiversifier.calculateDiversityStats(diversified);
 ...
 */
export function exampleInterleaving() {
  ...
  const interleaved = resultDiversifier.interleaveGroups(grouped);
