...
export async function exampleGetStats() {
  ...
  try {
    ...
    await faissVectorStore.reset(1536);
 ...
 */
export async function exampleAddVectors() {
...
export async function exampleSearch() {
  ...
  try {
    ...
    const results = await faissVectorStore.search(queryVector, 3);
...
export async function exampleSaveLoad() {
  ...
  try {
    ...
    await faissVectorStore.reset(1536);
    faissVectorStore.setIndexPath('./data/faiss-test');

    const vectors = Array.from({ length: 3 }, () =>
      Array.from({ length: 1536 }, () => Math.random() - 0.5)
    );
    ...
    const statsBefore = faissVectorStore.getStats();
    ...
    // Reset and load
    await faissVectorStore.reset();
    await faissVectorStore.load();

    console.log('  After load:');
    const statsAfter = faissVectorStore.getStats();
 ...
 */
export async function exampleEmbeddingIntegration() {
  ...
  try {
    ...
    // NOTE: This requires Ollama to be running (ollama serve)
 ...
 */
export async function examplePerformanceBenchmark() {
  ...
  try {
    ...
    await faissVectorStore.reset(1536);
    await faissVectorStore.addVectors(vectors, ids);
    ...
    for (let i = 0; i < iterations; i++) {
      ...
      await faissVectorStore.search(queryVector, 10);
