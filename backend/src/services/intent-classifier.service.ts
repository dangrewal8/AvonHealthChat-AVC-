...
interface KeywordPattern {
  keywords: string[];
  weight: number; // Multiplier for confidence calculation
}
...
class IntentClassifier {
   ...
   */
  private readonly intentPatterns: Record<QueryIntent, KeywordPattern[]> = {
   ...
   */
  private readonly specialPatterns: Array<{
    pattern: RegExp;
    intent: QueryIntent;
    confidence: number;
  }> = [
    {
      // Comparison patterns
      pattern:
        /what (changed|differences?|variations?)|how (did|has).*(changed?|differ|vary)|compare.*(?:with|to|between)/i,
    ...
    },
    {
      // Medication patterns
      pattern:
        /what (medications?|drugs?|prescriptions?)|list.*medications?|medications?.*taking/i,
   ...
   */
  private readonly AMBIGUITY_THRESHOLD = 0.15;
  ...
  classifyIntent(query: string): IntentClassification {
    ...
    const ambiguousIntents = sortedIntents.slice(1).filter((intentScore) => {
      return (
        topIntent.confidence - intentScore.confidence <= this.AMBIGUITY_THRESHOLD &&
  ...
  ): { intent: QueryIntent; confidence: number } | null {
   ...
   */
  private calculateKeywordScores(normalizedQuery: string): Record<QueryIntent, number> {
    ...
    for (const [intent, patterns] of Object.entries(this.intentPatterns)) {
      ...
      if (maxPossibleScore > 0) {
        scores[intent as QueryIntent] = Math.min(totalScore / maxPossibleScore, 1.0);
      }
    }
   ...
   */
  private containsKeyword(query: string, keyword: string): boolean {
   ...
   */
  private getMatchedKeywords(normalizedQuery: string, intent: QueryIntent): string[] {
  ...
  getIntentKeywords(): Record<QueryIntent, string[]> {
    ...
    for (const [intent, patterns] of Object.entries(this.intentPatterns)) {
      ...
      keywords[intent as QueryIntent] = allKeywords;
    }
