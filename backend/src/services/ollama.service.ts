...
You must think step-by-step, be honest about gaps, and help users find relevant information even when exact match isn't available.`;
    ...
    if (patientData.conditions && patientData.conditions.length > 0) {
      ...
      patientData.conditions.forEach((condition, idx) => {
        ...
        if (code && displayName !== code) fullContext += ` (ICD-10: ${code})`;
        fullContext += `\n`;
        if (condition.active !== undefined) fullContext += `   Status: ${condition.active ? 'Active' : 'Inactive'}\n`;
        if (condition.onset_date) fullContext += `   Onset: ${condition.onset_date}\n`;
        if (condition.end_date) fullContext += `   End Date: ${condition.end_date}\n`;
        if (condition.comment) fullContext += `   Note: ${condition.comment}\n`;
        if (condition.created_by) fullContext += `   Documented by: ${condition.created_by}\n`;
    ...
    if (patientData.medications && patientData.medications.length > 0) {
      ...
      activeMeds.forEach((med, idx) => {
        ...
        if (med.dose_form) medDisplay += ` ${med.dose_form}`;
        ...
        if (med.quantity) fullContext += `   Quantity: ${med.quantity}`;
        if (med.refills !== null && med.refills !== undefined) fullContext += ` (${med.refills} refills remaining)`;
        if (med.quantity || med.refills) fullContext += `\n`;

        // Timeline
        if (med.start_date) fullContext += `   Started: ${med.start_date}\n`;
        if (med.last_filled_at) fullContext += `   Last Filled: ${med.last_filled_at}\n`;

        // Prescriber
        if (med.created_by) fullContext += `   Prescribed by: ${med.created_by}\n`;

        // Status if available
        if (med.status) fullContext += `   Status: ${med.status}\n`;
    ...
    if (patientData.appointments && patientData.appointments.length > 0) {
      ...
      patientData.appointments.slice(0, 10).forEach((appt, idx) => {
        ...
        if (apptType) fullContext += ` (${apptType})`;
        fullContext += `\n`;

        // Description provides context
        if (appt.description) fullContext += `   Description: ${appt.description}\n`;

        // Interaction type (in-person, telehealth, etc.)
        if (interaction) fullContext += `   Type: ${interaction}\n`;

        // Timing
        if (appt.start_time) fullContext += `   Scheduled: ${appt.start_time}`;
        if (appt.end_time) fullContext += ` to ${appt.end_time}`;
        if (appt.start_time) fullContext += `\n`;

        // Actual times if different
        if (appt.actual_start_time) fullContext += `   Actual Start: ${appt.actual_start_time}\n`;
        if (appt.actual_end_time) fullContext += `   Actual End: ${appt.actual_end_time}\n`;

        // Provider/host
        if (appt.host || appt.provider) fullContext += `   Provider: ${appt.host || appt.provider}\n`;
        ...
        if (appt.status_history && appt.status_history.length > 0) {
          ...
          if (currentStatus.status) fullContext += `   Status: ${currentStatus.status}\n`;
        }

        // Visit note reference
        if (appt.visit_note) fullContext += `   Visit Note: ${appt.visit_note}\n`;
    ...
    if (patientData.documents && patientData.documents.length > 0) {
      ...
      patientData.documents.slice(0, 10).forEach((doc, idx) => {
        ...
        if (doc.filename) fullContext += ` - ${doc.filename}`;
        fullContext += `\n`;

        // Document template info
        if (doc.document_template) fullContext += `   Template: ${doc.document_template}\n`;

        // Created info
        if (doc.created_at) fullContext += `   Created: ${doc.created_at}`;
        if (doc.created_by) fullContext += ` by ${doc.created_by}`;
        if (doc.created_at) fullContext += `\n`;
    ...
    if (patientData.form_responses && patientData.form_responses.length > 0) {
      ...
      patientData.form_responses.slice(0, 10).forEach((form, idx) => {
        ...
        if (form.form_version) fullContext += ` (v${form.form_version})`;
        fullContext += `\n`;
        ...
        // Completion info
        if (form.created_at) fullContext += `   Completed: ${form.created_at}`;
        if (form.created_by) fullContext += ` by ${form.created_by}`;
        if (form.created_at) fullContext += `\n`;
    ...
    if (patientData.insurance_policies && patientData.insurance_policies.length > 0) {
      ...
      patientData.insurance_policies.forEach((policy, idx) => {
        ...
        if (policy.carrier_name) fullContext += `   Carrier: ${policy.carrier_name}\n`;
        if (policy.plan_name) fullContext += `   Plan: ${policy.plan_name}\n`;
        if (policy.policy_number) fullContext += `   Policy Number: ${policy.policy_number}\n`;
        if (policy.group_number) fullContext += `   Group Number: ${policy.group_number}\n`;

        // Coverage dates
        if (policy.effective_date) fullContext += `   Effective: ${policy.effective_date}`;
        if (policy.expiration_date) fullContext += ` to ${policy.expiration_date}`;
        if (policy.effective_date) fullContext += `\n`;

        // Subscriber info
        if (policy.subscriber_name) fullContext += `   Subscriber: ${policy.subscriber_name}\n`;
        if (policy.relationship_to_subscriber) {
          fullContext += `   Relationship: ${policy.relationship_to_subscriber}\n`;
        }

        // Created info
        if (policy.created_at) fullContext += `   Added: ${policy.created_at}`;
        if (policy.created_by) fullContext += ` by ${policy.created_by}`;
        if (policy.created_at) fullContext += `\n`;
