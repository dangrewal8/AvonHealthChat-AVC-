...
export interface ReRankingResult {
  ...
  signals: {
    ...
    query_overlap: number; // 0-1
    position_boost: number; // 0-1
    type_match_bonus: number; // 0-1
  };
  rank_change?: number; // Position change after re-ranking
}
...
interface ReRankingWeights {
  original_score: number; // Default: 0.7
  entity_coverage: number; // Default: 0.15
  query_overlap: number; // Default: 0.10
  type_match_bonus: number; // Default: 0.05
}
 ...
 */
class ReRanker {
  private readonly DEFAULT_WEIGHTS: ReRankingWeights = {
    original_score: 0.7,
  ...
  private readonly TOP_K_FOR_RERANKING = 20; // Only re-rank top 20

  /**
   * Intent to artifact type preference for re-ranking
   */
  private readonly INTENT_TYPE_BONUS: Record<QueryIntent, Record<string, number>> = {
  ...
  rerank(
    ...
    topK: number = this.TOP_K_FOR_RERANKING
  ): RetrievalCandidate[] {
    ...
    const reranked = candidatesToRerank.map((candidate, index) => {
      ...
      const typeMatchBonus = this.calculateTypeMatchBonus(
        candidate.chunk.metadata.artifact_type,
        query.intent
      );

      // Combine scores with weights
      const rerankScore = this.combineScores(
        originalScore,
        entityCoverage,
        queryOverlap,
        typeMatchBonus
  ...
  rerankWithDetails(
    ...
    const results = candidatesToRerank.map((candidate, index) => {
      ...
      const type_match_bonus = this.calculateTypeMatchBonus(
        candidate.chunk.metadata.artifact_type,
        query.intent
      );

      // Combined score
      const rerank_score = this.combineScores(
        originalScore,
        entity_coverage,
        query_overlap,
        type_match_bonus
  ...
  boostByEntityMatch(chunk: Chunk, entities: Entity[]): number {
    ...
    for (const entity of entities) {
      ...
      if (
        contentLower.includes(entity.text.toLowerCase()) ||
   ...
   */
  private calculatePositionBoost(position: number, totalResults: number): number {
    if (totalResults <= 1) {
      return 1.0;
    }
   ...
   */
  private calculateTypeMatchBonus(artifactType: string, intent: QueryIntent): number {
  ...
  private combineScores(
    ...
    weights: ReRankingWeights = this.DEFAULT_WEIGHTS
  ): number {
    return (
      weights.original_score * originalScore +
   ...
   */
  compareRankings(
    originalCandidates: RetrievalCandidate[],
    rerankedCandidates: RetrievalCandidate[]
  ): {
    improved: number;
    degraded: number;
    unchanged: number;
    top_changes: Array<{ chunk_id: string; old_rank: number; new_rank: number }>;
  } {
    const changes = {
      improved: 0,
      degraded: 0,
      unchanged: 0,
      top_changes: [] as Array<{ chunk_id: string; old_rank: number; new_rank: number }>,
    };
