...
import twoPassGenerator from './two-pass-generator.service';
...
export interface GeneratedAnswer {
  ...
  structured_extractions: Extraction[]; // Validated extractions
  model: string; // Model used (e.g., meditron)
  total_tokens: number; // Total tokens consumed
  generation_time_ms: number; // Total generation time
}
...
class AnswerGenerationAgent {
    ...
    try {
      ...
      if (!candidates || candidates.length === 0) {
        throw new AnswerGenerationError(
          'No candidates provided for answer generation',
          'NO_CANDIDATES'
        );
      }
      ...
      const twoPassResult = await twoPassGenerator.generateAnswer(candidates, query);

      // Step 2: Validate extractions
      const validationResult = this.validateExtractions(
      ...
      throw new AnswerGenerationError(
        `Answer generation failed: ${(error as Error).message}`,
        'GENERATION_FAILED',
        { originalError: error }
      );
    }
  ...
  validateExtractions(
    ...
    extractions.forEach((extraction, index) => {
      ...
      if (end > chunkContent.length) {
        errors.push(
          `Extraction ${index}: char_offsets [${start}, ${end}] exceed chunk length ${chunkContent.length}`
        );
        return;
      }
      ...
      if (extractedText !== supporting_text) {
        warnings.push(
          `Extraction ${index}: Supporting text mismatch (expected "${extractedText}", got "${supporting_text}")`
        );
      }
      ...
      if (candidate.chunk.artifact_id !== artifact_id) {
        errors.push(
          `Extraction ${index}: artifact_id mismatch (chunk has ${candidate.chunk.artifact_id}, extraction has ${artifact_id})`
        );
      }
    });
  ...
  getExtractionStats(extractions: Extraction[]): {
    ...
    if (stats.with_provenance > 0) {
      stats.avg_supporting_text_length = Math.round(
        totalSupportingTextLength / stats.with_provenance
      );
    }
