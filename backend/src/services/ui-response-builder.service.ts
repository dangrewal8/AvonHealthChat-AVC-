...
export interface QueryMetrics {
  query_timestamp: string; // ISO timestamp
...
export interface ResponseMetadata {
  ...
  query_timestamp: string; // When query was received
  response_timestamp: string; // When response was generated
...
export interface AuditInfo {
  ...
  components_executed: string[]; // Pipeline components executed
...
export interface UIResponse {
  query_id: string; // Unique query identifier
  short_answer: string; // Short answer (< 50 words)
  detailed_summary: string; // Detailed summary (< 200 words)
  structured_extractions: Extraction[]; // Structured extractions with provenance
  provenance: FormattedProvenance[]; // Formatted citations for UI
  confidence: ConfidenceScore; // Confidence score with components
  metadata: ResponseMetadata; // Response metadata
  audit: AuditInfo; // Audit information
}
...
class UIResponseBuilder {
   ...
   */
  private buildAuditInfo(query: StructuredQuery, metrics: QueryMetrics): AuditInfo {
    ...
    return {
      ...
      timestamps: {
        ...
        retrieval_completed: new Date(
          new Date(metrics.query_timestamp).getTime() + metrics.retrieval_time_ms
        ).toISOString(),
        generation_started: new Date(
          new Date(metrics.query_timestamp).getTime() + metrics.retrieval_time_ms
        ).toISOString(),
        generation_completed: new Date(
          new Date(metrics.query_timestamp).getTime() +
   ...
   * @returns JSON string
   */
  toJSON(response: UIResponse | ErrorResponse, pretty: boolean = false): string {
   ...
   */
  isResponseTooLarge(response: UIResponse | ErrorResponse, maxSizeBytes: number = 1048576): boolean {
   ...
   */
  truncateResponse(response: UIResponse, maxSizeBytes: number = 1048576): UIResponse {
