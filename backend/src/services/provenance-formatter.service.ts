...
export interface FormattedProvenance {
  ...
  artifact_type: string; // Type of artifact (care_plan, note, etc.)
  snippet: string; // Cited text with 50 chars context
  note_date: string; // Formatted date (relative or absolute)
  author?: string; // Author of the artifact
  source_url: string; // URL to view full artifact
  char_offsets: [number, number]; // Original char offsets
...
class ProvenanceFormatter {
   ...
   */
  private readonly CONTEXT_SIZE = 50;

  /**
   * Max snippet length
   */
  private readonly MAX_SNIPPET_LENGTH = 200;

  /**
   * Relative date threshold (days)
   */
  private readonly RELATIVE_DATE_THRESHOLD_DAYS = 7;
  ...
  format(
    ...
    const formattedProvenances: FormattedProvenance[] = [];
  ...
  extractSnippet(text: string, offsets: [number, number]): string {
    ...
    const extendedBoundaries = this.extendToSentenceBoundaries(
      text,
      snippetStart,
      snippetEnd
    );
   ...
   */
  groupByArtifact(
    provenances: FormattedProvenance[]
  ): Map<string, FormattedProvenance[]> {
    const grouped = new Map<string, FormattedProvenance[]>();
