...
class ChunkStore {
   ...
   */
  private chunks: Map<string, ParsedChunk> = new Map();
   ...
   */
  async store(chunks: ParsedChunk[]): Promise<StoreResult> {
   ...
   */
  async retrieve(chunkId: string): Promise<ParsedChunk | null> {
   ...
   */
  async query(filters: ChunkFilter = {}): Promise<ParsedChunk[]> {
    ...
    if (filters.date_from || filters.date_to) {
      const dateChunks = this.getChunksInDateRange(
        filters.date_from,
        filters.date_to
      );
    ...
    if (filters.artifact_type) {
      chunks = chunks.filter(
        (chunk) => chunk.artifact_type === filters.artifact_type
      );
    }
    ...
    if (filters.entity_type) {
      chunks = chunks.filter((chunk) =>
        chunk.entities.some((entity) => entity.type === filters.entity_type)
      );
    }
    ...
    if (filters.entity_text) {
      ...
      chunks = chunks.filter((chunk) =>
        chunk.entities.some(
          (entity) =>
            entity.text.toLowerCase().includes(searchText) ||
   ...
   */
  async getByPatient(patientId: string): Promise<ParsedChunk[]> {
   ...
   * @returns Array of chunks
   */
  async getByArtifact(artifactId: string): Promise<ParsedChunk[]> {
   ...
   */
  async deleteByPatient(patientId: string): Promise<number> {
   ...
   * @returns Number of chunks deleted
   */
  async deleteByArtifact(artifactId: string): Promise<number> {
   ...
   */
  async garbageCollect(olderThan: string): Promise<number> {
  ...
  async getStatistics(): Promise<StorageStatistics> {
    ...
    for (const chunk of this.chunks.values()) {
      ...
      // Type-safe increment
      const artifactType = chunk.artifact_type as 'care_plan' | 'medication' | 'note';
      byType[artifactType]++;
   ...
   */
  private updateIndexes(chunk: ParsedChunk): void {
    // Artifact index
   ...
   */
  private removeFromIndexes(chunk: ParsedChunk): void {
    // Artifact index
   ...
   */
  private validateChunk(chunk: ParsedChunk): void {
    if (!chunk.chunk_id) {
