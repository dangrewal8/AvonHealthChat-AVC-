...
/**
 * Retrieval candidate (for snippets)
 */
interface Candidate {
  artifact_id: string;
  snippet: string;
...
class PartialResultsHandler {
  ...
  handlePartialResult(error: Error, context: PipelineContext): PartialResult {
    ...
    if (context.data.structured_query) completed.push('query_understanding');
    if (context.data.retrieval_results) completed.push('retrieval');
    if (context.data.extractions) completed.push('extraction');
    if (context.data.generated_answer) completed.push('generation');
        ...
        partial.available_data.candidates
          ...
          .map(
            (c: Candidate, i: number) =>
              `${i + 1}. ${c.snippet} (Score: ${c.score.toFixed(2)})`
          )
          .join('\n\n');
  ...
  hasPartialResults(context: PipelineContext): boolean {
    return (
      context.data.structured_query !== undefined ||
