...
interface ChunkIndexes {
  byPatient: Map<string, Set<string>>; // patient_id → chunk_ids
  byType: Map<string, Set<string>>; // artifact_type → chunk_ids
  byAuthor: Map<string, Set<string>>; // author → chunk_ids
  byDate: Map<string, Set<string>>; // date → chunk_ids
...
class MetadataFilter {
  ...
  buildIndexes(chunks: Chunk[]): void {
    ...
    console.log(
      `Indexes built for ${chunks.length} chunks in ${endTime - startTime}ms`
    );
  }
  ...
  applyFiltersWithStats(chunks: Chunk[], filters: FilterCriteria): FilterResult {
    ...
    if (filters.patient_id) filtersApplied.push('patient_id');
    if (filters.artifact_types) filtersApplied.push('artifact_types');
    if (filters.date_from || filters.date_to) filtersApplied.push('date_range');
    if (filters.author) filtersApplied.push('author');
  ...
  applyFiltersWithIndexes(filters: FilterCriteria): Chunk[] {
    ...
    if (filters.artifact_types && filters.artifact_types.length > 0) {
      const typeChunks = new Set<string>();
