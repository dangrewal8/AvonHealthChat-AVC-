...
export interface OllamaLLMConfig {
  ...
  model: string; // 'llama3', 'meditron', 'biomistral'
  maxTokens: number; // Maximum response tokens (num_predict)
...
export interface ExtractionResult {
  extractions: unknown[]; // Extracted data (flexible structure)
 ...
 * LLM Service Error
 */
export class OllamaLLMServiceError extends Error {
  constructor(
    message: string,
    public code: string,
...
class OllamaLLMService {
  ...
  private config: OllamaLLMConfig;
   ...
   */
  private readonly EXTRACTION_SYSTEM_PROMPT = `You are a medical information extraction assistant.
...
5. Be concise and clear`;
  ...
  constructor(config?: Partial<OllamaLLMConfig>) {
    ...
    const defaultConfig: OllamaLLMConfig = {
...
IMPORTANT: Respond with valid JSON only. No explanations, no markdown, just pure JSON.`;
    ...
    try {
      ...
      try {
        ...
        if (jsonMatch) {
          ...
          throw new OllamaLLMServiceError(
            `JSON parse error: ${(parseError as Error).message}\nResponse: ${generatedText.substring(0, 200)}`,
            'JSON_PARSE_ERROR',
            false
          );
        }
   ...
   * @returns Generated text
   */
  async generateWithRetry(prompt: string, options?: GenerateOptions): Promise<string> {
  ...
  async extractWithRetries(
    ...
    return this.withRetries(
      () => this.extract(systemPrompt, userPrompt, temperature),
      maxRetries
    );
  }
  ...
  async summarizeWithRetries(
    ...
    return this.withRetries(
      () => this.summarize(systemPrompt, userPrompt, temperature),
      maxRetries
    );
  }
  ...
  private async withRetries<T>(fn: () => Promise<T>, maxRetries: number): Promise<T> {
    let lastError: OllamaLLMServiceError | null = null;

    for (let attempt = 1; attempt <= maxRetries; attempt++) {
      try {
        return await fn();
      } catch (error) {
        lastError = error as OllamaLLMServiceError;
        ...
        console.error(
          `[Ollama LLM Service] Attempt ${attempt}/${maxRetries} failed:`,
          lastError.message
        );
   ...
   */
  private handleGenerationError(error: any, method: string): never {
    ...
    throw new OllamaLLMServiceError(
      `LLM generation failed: ${error.message || 'Unknown error'}`,
      'GENERATION_ERROR',
      true
    );
  }
  ...
  async verifyModel(): Promise<boolean> {
    try {
      ...
      return models.some(
        (m: any) => m.name === this.config.model || m.name === `${this.config.model}:latest`
      );
    } catch (error) {
