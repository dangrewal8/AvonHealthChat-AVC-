...
const validResult = artifactValidator.validate(validArtifact);
console.log('  Valid:', validResult.valid ? '✅' : '❌');
console.log('  Errors:', validResult.errors.length);
console.log('  Warnings:', validResult.warnings.length);
console.log('  Summary:', artifactValidator.getValidationSummary(validResult));
console.log('');
...
const missingResult = artifactValidator.validate(missingFieldsArtifact);
...
const dateResult = artifactValidator.validate(invalidDateArtifact);
...
const hasDateError = dateResult.errors.some(
  (e) => e.field === 'occurred_at' && e.message.includes('ISO 8601')
);
if (hasDateError) {
...
const futureResult = artifactValidator.validate(futureArtifact);
...
const hasFutureWarning = futureResult.warnings.some(
  (w) => w.field === 'occurred_at' && w.message.includes('future')
);
...
const typeResult = artifactValidator.validate(wrongTypeArtifact);
...
const hasTypeError = typeResult.errors.some(
  (e) => e.field === 'id' && e.message.includes('must be a string')
);
