...
exports.exampleBasicExpansion = exampleBasicExpansion;
exports.exampleSynonymLookup = exampleSynonymLookup;
exports.exampleEntityBasedExpansion = exampleEntityBasedExpansion;
exports.exampleMedicationExpansion = exampleMedicationExpansion;
exports.exampleConditionExpansion = exampleConditionExpansion;
exports.exampleExpandedSearchTerms = exampleExpandedSearchTerms;
exports.exampleAbbreviationExpansion = exampleAbbreviationExpansion;
exports.exampleSynonymAvailability = exampleSynonymAvailability;
exports.exampleBatchExpansion = exampleBatchExpansion;
exports.exampleDictionaryInspection = exampleDictionaryInspection;
exports.exampleRetrievalIntegration = exampleRetrievalIntegration;
exports.exampleComplexQueryExpansion = exampleComplexQueryExpansion;
exports.runAllExamples = runAllExamples;
const query_expander_service_1 = __importDefault(require("../services/query-expander.service"));
 ...
 */
function exampleBasicExpansion() {
    ...
    queries.forEach((query, i) => {
        ...
        // Expand query
        const expanded = query_expander_service_1.default.expandQuery(query, entities);
...
function exampleSynonymLookup() {
    ...
    terms.forEach((term, i) => {
        ...
        const synonyms = query_expander_service_1.default.getMedicalSynonyms(term);
 ...
 */
function exampleEntityBasedExpansion() {
 ...
 */
function exampleMedicationExpansion() {
 ...
 */
function exampleConditionExpansion() {
 ...
 */
function exampleExpandedSearchTerms() {
    ...
    terms.forEach((term, i) => {
        console.log(`  Term ${i + 1}: "${term}"`);
        const synonyms = query_expander_service_1.default.getMedicalSynonyms(term);
        const searchTerms = query_expander_service_1.default.buildExpandedSearchTerms(term, synonyms);
 ...
 */
function exampleSynonymAvailability() {
    ...
    terms.forEach(({ term, expectSynonyms }, i) => {
        console.log(`  Term ${i + 1}: "${term}"`);
        const hasSynonyms = query_expander_service_1.default.hasSynonyms(term);
        const synonymCount = query_expander_service_1.default.getSynonymCount(term);
 ...
 */
function exampleBatchExpansion() {
    ...
    const batchInput = queries.map((query) => ({
        query,
        entities: entity_extractor_service_1.default.extractEntities(query),
    }));
    // Expand in batch
    const results = query_expander_service_1.default.expandBatch(batchInput);
 ...
 */
function exampleDictionaryInspection() {
    console.log('Example 10: Dictionary Inspection');
    console.log('-'.repeat(80));
    const allTerms = query_expander_service_1.default.getAllMedicalTerms();
    console.log(`  Medical synonym dictionary contains ${allTerms.length} terms\n`);
    // Show sample terms
    console.log('  Sample medications:');
    const medications = allTerms.filter((t) => t === 'ibuprofen' || t === 'metformin' || t === 'lisinopril');
    medications.forEach((term) => {
        console.log(`    - ${term} (${query_expander_service_1.default.getSynonymCount(term)} synonyms)`);
    });
    console.log('');
    console.log('  Sample conditions:');
    const conditions = allTerms.filter((t) => t === 'hypertension' || t === 'diabetes' || t === 'heart failure');
 ...
 */
function exampleRetrievalIntegration() {
 ...
 */
function exampleComplexQueryExpansion() {
    console.log('Example 12: Complex Medical Query Expansion');
    console.log('-'.repeat(80));
    const query = 'Patient with type 2 diabetes and hypertension on metformin and lisinopril, ' +
    ...
    console.log(`    - Original: 1 query → ${expanded.expanded_terms.length} variations`);
    console.log(`    - Synonyms: ${Object.keys(expanded.synonym_map).length} terms expanded`);
...
// runAllExamples();
//# sourceMappingURL=query-expander.example.js.map