...
exports.exampleRelativeDates = exampleRelativeDates;
exports.exampleAbsoluteDates = exampleAbsoluteDates;
exports.exampleAbsoluteFormats = exampleAbsoluteFormats;
exports.exampleTimeAgo = exampleTimeAgo;
exports.exampleDateTime = exampleDateTime;
exports.exampleDateRange = exampleDateRange;
exports.exampleISODate = exampleISODate;
exports.exampleIsToday = exampleIsToday;
exports.exampleProvenanceFormatting = exampleProvenanceFormatting;
exports.exampleMedicalRecords = exampleMedicalRecords;
exports.exampleTimeZones = exampleTimeZones;
exports.exampleProvenanceIntegration = exampleProvenanceIntegration;
exports.exampleEdgeCases = exampleEdgeCases;
exports.exampleRealWorld = exampleRealWorld;
exports.exampleExplain = exampleExplain;
exports.runAllExamples = runAllExamples;
const date_formatter_service_1 = __importDefault(require("../services/date-formatter.service"));
/**
 * Example 1: Relative dates (< 7 days)
 */
function exampleRelativeDates() {
    ...
    dates.forEach(({ label, date }) => {
        const formatted = date_formatter_service_1.default.formatRelative(date, now);
        console.log(`    ${label}:`);
 ...
 */
function exampleAbsoluteFormats() {
    console.log('Example 3: Absolute Date Formats');
    console.log('-'.repeat(80));
    const date = '2025-01-15T10:30:00Z';
    const shortFormat = date_formatter_service_1.default.formatAbsolute(date, 'short');
    const longFormat = date_formatter_service_1.default.formatAbsolute(date, 'long');
...
function exampleTimeAgo() {
    ...
    dates.forEach(date => {
        const timeAgo = date_formatter_service_1.default.getTimeAgo(date, now);
...
function exampleDateTime() {
    ...
    const shortDateTime = date_formatter_service_1.default.formatDateTime(date, 'short');
    const longDateTime = date_formatter_service_1.default.formatDateTime(date, 'long');
    console.log('  Date:', date, '\n');
...
function exampleDateRange() {
    ...
    ranges.forEach(({ label, start, end }) => {
        const shortRange = date_formatter_service_1.default.formatDateRange(start, end, 'short');
        const longRange = date_formatter_service_1.default.formatDateRange(start, end, 'long');
        console.log(`    ${label}:`);
...
function exampleISODate() {
    ...
    dates.forEach(date => {
        const iso = date_formatter_service_1.default.formatISODate(date);
...
function exampleIsToday() {
    ...
    dates.forEach(({ date, label }) => {
        const isToday = date_formatter_service_1.default.isToday(date, now);
        const isYesterday = date_formatter_service_1.default.isYesterday(date, now);
 ...
 */
function exampleProvenanceFormatting() {
    ...
    provenances.forEach(({ artifact_id, occurred_at }) => {
        const formatted = date_formatter_service_1.default.formatForProvenance(occurred_at);
 ...
 */
function exampleMedicalRecords() {
    ...
    records.forEach(({ type, date }) => {
        const display = date_formatter_service_1.default.formatRelative(date, now);
        console.log(`    ${type}:`);
...
function exampleProvenanceIntegration() {
    ...
    const formatted = date_formatter_service_1.default.formatForProvenance(provenance.occurred_at);
...
function exampleRealWorld() {
    ...
    console.log(`    Display: "${date_formatter_service_1.default.formatRelative(visitDate, now)}"\n`);
    console.log('  Previous visit:');
    console.log(`    Occurred: ${previousVisit}`);
    console.log(`    Display: "${date_formatter_service_1.default.formatRelative(previousVisit, now)}"\n`);
    console.log('  Care plan created:');
    console.log(`    Occurred: ${carePlanDate}`);
    console.log(`    Display: "${date_formatter_service_1.default.formatRelative(carePlanDate, now)}"\n`);
...
function exampleExplain() {
    ...
    console.log('-'.repeat(80));
    const explanation = date_formatter_service_1.default.explain();
    console.log('\n' + explanation.split('\n').map(line => `  ${line}`).join('\n'));
    console.log('\n  ✅ Success\n');
}
/**
...
// runAllExamples();
//# sourceMappingURL=date-formatter.example.js.map