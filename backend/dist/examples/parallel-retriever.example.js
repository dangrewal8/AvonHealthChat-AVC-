...
exports.exampleAutoParallelization = exampleAutoParallelization;
exports.exampleTimeWindowParallelization = exampleTimeWindowParallelization;
exports.examplePerformanceComparison = examplePerformanceComparison;
exports.exampleMergingDeduplication = exampleMergingDeduplication;
exports.exampleRetrievalSummary = exampleRetrievalSummary;
exports.exampleSingleType = exampleSingleType;
exports.exampleBatchParallel = exampleBatchParallel;
exports.exampleForceParallel = exampleForceParallel;
...
const parallel_retriever_service_1 = __importDefault(require("../services/parallel-retriever.service"));
 ...
 */
async function exampleAutoParallelization() {
    ...
    const result = await parallel_retriever_service_1.default.retrieve(query);
    console.log('  Results:\n');
 ...
 */
async function exampleTimeWindowParallelization() {
...
async function examplePerformanceComparison() {
    ...
    const comparison = await parallel_retriever_service_1.default.comparePerformance(query);
    ...
    console.log(`  Performance Gain: ${(((comparison.speedup - 1) * 100).toFixed(0))}%\n`);
    console.log('  ✅ Success\n');
}
/**
 * Example 4: Result merging and deduplication
 */
async function exampleMergingDeduplication() {
 ...
 */
async function exampleRetrievalSummary() {
    ...
    const result = await parallel_retriever_service_1.default.retrieve(query);
    console.log('\n' + parallel_retriever_service_1.default.getSummary(result));
    console.log('\n  ✅ Success\n');
}
/**
 * Example 6: Single type query (no parallelization)
 */
async function exampleSingleType() {
 ...
 */
async function exampleBatchParallel() {
    ...
    const results = await parallel_retriever_service_1.default.batchRetrieve(queries);
 ...
 */
async function exampleForceParallel() {
    ...
    const result1 = await parallel_retriever_service_1.default.retrieve(query, false);
    ...
    const result2 = await parallel_retriever_service_1.default.retrieve(query, true);
...
async function exampleWithQueryUnderstanding() {
    ...
    const result = await parallel_retriever_service_1.default.retrieve(structuredQuery);
    console.log('  Results:');
    console.log(`    Parallel Searches: ${result.parallel_searches}`);
    console.log(`    Time: ${result.retrieval_time_ms}ms`);
    console.log(`    Candidates: ${result.candidates.length}\n`);
 ...
 */
async function examplePerformanceMetrics() {
...
// runAllExamples();
//# sourceMappingURL=parallel-retriever.example.js.map