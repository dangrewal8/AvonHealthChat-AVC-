 ...
 * - Automatic recovery
 * - Integration with OpenAI and Avon API
 * - Multiple service monitoring
 */
Object.defineProperty(exports, "__esModule", { value: true });
exports.exampleBasicUsage = exampleBasicUsage;
exports.exampleStateTransitions = exampleStateTransitions;
exports.exampleFailureThreshold = exampleFailureThreshold;
exports.exampleResetTimeout = exampleResetTimeout;
exports.exampleFailFast = exampleFailFast;
exports.exampleAutomaticRecovery = exampleAutomaticRecovery;
exports.exampleOpenAIIntegration = exampleOpenAIIntegration;
exports.exampleAvonAPIIntegration = exampleAvonAPIIntegration;
exports.exampleMultipleServices = exampleMultipleServices;
exports.exampleStatistics = exampleStatistics;
exports.exampleManualControl = exampleManualControl;
exports.exampleRealWorldScenario = exampleRealWorldScenario;
...
async function exampleBasicUsage() {
    ...
    const breaker = new circuit_breaker_service_1.CircuitBreaker({
        failureThreshold: 3,
        resetTimeoutMs: 5000,
    });
 ...
 */
async function exampleStateTransitions() {
 ...
 */
async function exampleFailureThreshold() {
    ...
    for (let i = 1; i <= 7; i++) {
        try {
            ...
            if (stats.state === circuit_breaker_service_1.CircuitState.OPEN) {
 ...
 */
async function exampleResetTimeout() {
 ...
 */
async function exampleFailFast() {
 ...
 */
async function exampleAutomaticRecovery() {
...
/**
 * Example 7: OpenAI API integration
 */
async function exampleOpenAIIntegration() {
    console.log('Example 7: OpenAI API Integration');
    console.log('-'.repeat(80));
    // Get circuit breaker for OpenAI
    const openaiBreaker = circuit_breaker_service_1.circuitBreakerManager.getBreaker('openai', {
        failureThreshold: 3,
        resetTimeoutMs: 60000, // 1 minute
    });
    console.log('  OpenAI Circuit Breaker:');
    console.log('    Failure threshold: 3');
    ...
    async function callOpenAI(prompt) {
        return openaiBreaker.execute(async () => {
            ...
            // Simulate API call
            // In production: return await openai.chat.completions.create({ ... })
    ...
    console.log('  Making OpenAI requests:\n');
    for (let i = 1; i <= 5; i++) {
        try {
            const result = await callOpenAI(`Query ${i}`);
            console.log(`    Request ${i}: SUCCESS`);
            console.log(`      ${result}`);
        }
        catch (error) {
            ...
            console.log(`      Circuit: ${openaiBreaker.getState()}`);
        }
    }
    const stats = openaiBreaker.getStats();
    console.log('\n  OpenAI Circuit Stats:');
    console.log('    State:', stats.state);
 ...
 */
async function exampleAvonAPIIntegration() {
    console.log('Example 8: Avon Health API Integration');
    console.log('-'.repeat(80));
    // Get circuit breaker for Avon API
    const avonBreaker = circuit_breaker_service_1.circuitBreakerManager.getBreaker('avon_api', {
        failureThreshold: 5,
    ...
    async function callAvonAPI(patientId) {
...
async function exampleMultipleServices() {
    ...
    const services = ['openai', 'avon_api', 'vector_db'];
    for (const service of services) {
        console.log(`  ${service.toUpperCase()}:`);
        for (let i = 1; i <= 3; i++) {
            try {
                await circuit_breaker_service_1.circuitBreakerManager.execute(service, async () => {
                    // Simulate random failure
                    if (Math.random() < 0.3) {
                        throw new Error(`${service} error`);
                    }
    ...
    const allStats = circuit_breaker_service_1.circuitBreakerManager.getAllStats();
    ...
    console.log('  Total services monitored:', circuit_breaker_service_1.circuitBreakerManager.getBreakerCount());
...
async function exampleRealWorldScenario() {
    ...
    console.log('-'.repeat(80));
    console.log('  Scenario: RAG pipeline with OpenAI and vector DB\n');
    // Create circuit breakers
    const openaiBreaker = circuit_breaker_service_1.circuitBreakerManager.getBreaker('openai', {
        failureThreshold: 3,
        resetTimeoutMs: 60000,
    });
    const vectorDbBreaker = circuit_breaker_service_1.circuitBreakerManager.getBreaker('vector_db', {
        failureThreshold: 5,
    ...
    const openaiStats = openaiBreaker.getStats();
    console.log('  OpenAI:');
    console.log('    State:', openaiStats.state);
    console.log('    Success rate:', (openaiBreaker.getSuccessRate() * 100).toFixed(1), '%');
...
async function runAllExamples() {
    ...
    try {
        ...
        await exampleOpenAIIntegration();
