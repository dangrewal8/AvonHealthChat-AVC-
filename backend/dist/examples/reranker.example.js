...
exports.exampleBasicReRanking = exampleBasicReRanking;
exports.exampleQueryRelevance = exampleQueryRelevance;
exports.exampleEntityCoverage = exampleEntityCoverage;
exports.exampleDetailedReRanking = exampleDetailedReRanking;
exports.exampleRankingComparison = exampleRankingComparison;
exports.exampleTypeMatchBonus = exampleTypeMatchBonus;
exports.exampleTopKReRanking = exampleTopKReRanking;
exports.exampleBatchReRanking = exampleBatchReRanking;
exports.exampleReRankingWeights = exampleReRankingWeights;
exports.exampleEntityImpact = exampleEntityImpact;
exports.exampleBeforeAfter = exampleBeforeAfter;
exports.runAllExamples = runAllExamples;
const reranker_service_1 = __importDefault(require("../services/reranker.service"));
...
function generateSampleCandidates() {
    const chunks = [
        ...
        },
        {
            ...
            patient_id: 'patient_123',
            content: 'Progress note: Patient continues diabetes treatment. No medication changes. ' +
        ...
        },
        {
            ...
            patient_id: 'patient_123',
            content: 'A1C level: 6.8%. Improved diabetes control with current medication regimen. ' +
        ...
        },
        {
            ...
            patient_id: 'patient_123',
            content: 'Patient education provided on medication adherence and glucose monitoring.',
            metadata: {
                artifact_type: 'progress_note',
        ...
        },
        {
            ...
            patient_id: 'patient_123',
            content: 'Care plan: Continue metformin 500mg BID. Schedule follow-up for diabetes management.',
            metadata: {
 ...
 */
function exampleBasicReRanking() {
    ...
    const reranked = reranker_service_1.default.rerank(candidates, query);
 ...
 */
function exampleQueryRelevance() {
    ...
    candidates.forEach((candidate) => {
        const relevance = reranker_service_1.default.calculateQueryRelevance(candidate.chunk, query);
 ...
 */
function exampleEntityCoverage() {
    ...
    console.log('');
    candidates.forEach((candidate) => {
        const coverage = reranker_service_1.default.boostByEntityMatch(candidate.chunk, entities);
        console.log(`  ${candidate.chunk.chunk_id}:`);
 ...
 */
function exampleDetailedReRanking() {
    console.log('Example 4: Detailed Re-Ranking Analysis');
    console.log('-'.repeat(80));
    const candidates = generateSampleCandidates();
    const query = generateSampleQuery();
    const results = reranker_service_1.default.rerankWithDetails(candidates, query);
    console.log('  Detailed Re-Ranking Results:\n');
    results.slice(0, 3).forEach((result, i) => {
        console.log(`  === Rank ${i + 1} ===`);
        console.log(reranker_service_1.default.explainReRanking(result));
        console.log('');
    });
    console.log('  ✅ Success\n');
}
 ...
 */
function exampleRankingComparison() {
    console.log('Example 5: Ranking Comparison');
    console.log('-'.repeat(80));
    const originalCandidates = generateSampleCandidates();
    const query = generateSampleQuery();
    const reranked = reranker_service_1.default.rerank(originalCandidates, query);
    const comparison = reranker_service_1.default.compareRankings(originalCandidates, reranked);
 ...
 */
function exampleTypeMatchBonus() {
    ...
    intents.forEach((intent) => {
        ...
        const reranked = reranker_service_1.default.rerank(candidates, query, 5);
...
function exampleTopKReRanking() {
    ...
    topKValues.forEach((k) => {
        ...
        reranker_service_1.default.rerank(candidates, query, k);
 ...
 */
function exampleBatchReRanking() {
    ...
    const queries = [
        ...
        },
        {
            ...
            patient_id: 'patient_123',
            intent: intent_classifier_service_1.QueryIntent.RETRIEVE_NOTES,
            entities: [],
            temporal_filter: null,
            filters: {},
            detail_level: 3,
    ...
    const reranked = reranker_service_1.default.batchRerank(candidateLists, queries);
 ...
 */
function exampleReRankingWeights() {
    console.log('Example 9: Re-Ranking Weights');
    console.log('-'.repeat(80));
    const weights = reranker_service_1.default.getDefaultWeights();
    ...
    const sum = weights.original_score +
...
function exampleEntityImpact() {
    ...
    const reranked1 = reranker_service_1.default.rerank(candidates, query1, 3);
    reranked1.forEach((c) => {
        console.log(`    ${c.rank}. ${c.chunk.chunk_id} - ${c.score.toFixed(3)}`);
    });
    console.log('\n  Scenario 2: With Entities (metformin, diabetes)\n');
    const reranked2 = reranker_service_1.default.rerank(candidates, query2, 3);
    reranked2.forEach((c) => {
        console.log(`    ${c.rank}. ${c.chunk.chunk_id} - ${c.score.toFixed(3)}`);
    });
    console.log('\n  ✅ Success\n');
...
// runAllExamples();
//# sourceMappingURL=reranker.example.js.map