 ...
 */
function createExampleArtifacts() {
    return [
        {
            id: (0, uuid_1.v4)(),
            type: 'care_plan',
 ...
 */
function createInvalidArtifacts() {
    return [
        // Missing patient_id
        {
            id: (0, uuid_1.v4)(),
 ...
 */
function displayChunkDetails(chunk, index) {
    console.log(`\n${'─'.repeat(80)}`);
    ...
    if (chunk.entities.length === 0) {
        ...
        for (const entity of chunk.entities) {
            console.log(`  - [${entity.type}] "${entity.text}" → "${entity.normalized}"`);
        }
    }
}
/**
...
function testBasicParsing() {
    ...
    const result = parser_agent_service_1.default.parseWithResult(artifacts);
...
function testBatchParsing() {
    ...
    displayResults(result);
    // Show statistics
    const stats = parser_agent_service_1.default.getStatistics(artifacts, result);
    console.log('\nStatistics:');
...
function testErrorHandling() {
    ...
    const result = parser_agent_service_1.default.parseWithResult(mixedArtifacts);
    displayResults(result);
    console.log('\nError Details:');
    for (const error of result.errors) {
...
function testEntityFiltering() {
    ...
    const medicationChunks = parser_agent_service_1.default.filterByEntityType(chunks, 'medication');
    ...
    // Filter by condition
    const conditionChunks = parser_agent_service_1.default.filterByEntityType(chunks, 'condition');
    ...
    console.log(`${'─'.repeat(80)}`);
    const metforminChunks = parser_agent_service_1.default.findChunksWithEntity(chunks, 'metformin');
...
function testUniqueEntities() {
    ...
    const artifacts = createExampleArtifacts();
    const chunks = parser_agent_service_1.default.parse(artifacts);
    const uniqueEntities = parser_agent_service_1.default.getUniqueEntities(chunks);
...
function testBatchProcessing() {
    ...
    const chunks = parser_agent_service_1.default.parseInBatches(largeDataset, 10);
...
async function main() {
    ...
    console.log('='.repeat(80));
    console.log('\n' + parser_agent_service_1.default.explain());
    console.log('\n' + '='.repeat(80));
...
//# sourceMappingURL=parser-agent.example.js.map