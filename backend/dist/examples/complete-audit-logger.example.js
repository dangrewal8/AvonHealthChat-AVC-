...
exports.exampleLogComplete = exampleLogComplete;
exports.exampleLogMultiple = exampleLogMultiple;
exports.exampleQueryByPatient = exampleQueryByPatient;
exports.exampleQueryByUser = exampleQueryByUser;
exports.exampleGetByQueryId = exampleGetByQueryId;
exports.examplePrivacyFull = examplePrivacyFull;
exports.examplePrivacyRedacted = examplePrivacyRedacted;
exports.examplePrivacyMinimal = examplePrivacyMinimal;
exports.exampleFilterByDateRange = exampleFilterByDateRange;
exports.exampleFilterByModel = exampleFilterByModel;
...
const complete_audit_logger_service_1 = __importStar(require("../services/complete-audit-logger.service"));
/**
 * Generate sample complete audit entry
 */
function generateSampleEntry(userId, patientId, queryText) {
 ...
 */
async function exampleLogComplete() {
    ...
    await complete_audit_logger_service_1.default.logComplete(entry);
 ...
 */
async function exampleLogMultiple() {
 ...
 */
async function exampleQueryByPatient() {
    console.log('Example 3: Query by Patient ID');
    console.log('-'.repeat(80));
    const results = await complete_audit_logger_service_1.default.query({
 ...
 */
async function exampleQueryByUser() {
 ...
 */
async function exampleGetByQueryId() {
    ...
    await complete_audit_logger_service_1.default.logComplete(entry);
    // Then retrieve it
    const retrieved = await complete_audit_logger_service_1.default.getByQueryId(entry.query_id);
    if (retrieved) {
 ...
 */
async function examplePrivacyFull() {
    console.log('Example 6: Privacy Mode - Full');
    console.log('-'.repeat(80));
    const results = await complete_audit_logger_service_1.default.query({
        patient_id: 'patient_456',
        limit: 1,
        privacy_mode: complete_audit_logger_service_1.PrivacyMode.FULL,
    });
    if (results.length > 0) {
 ...
 */
async function examplePrivacyRedacted() {
    console.log('Example 7: Privacy Mode - Redacted');
    console.log('-'.repeat(80));
    const results = await complete_audit_logger_service_1.default.query({
        patient_id: 'patient_456',
        limit: 1,
        privacy_mode: complete_audit_logger_service_1.PrivacyMode.REDACTED,
    });
    if (results.length > 0) {
        const entry = results[0];
 ...
 */
async function examplePrivacyMinimal() {
    console.log('Example 8: Privacy Mode - Minimal');
    console.log('-'.repeat(80));
    const results = await complete_audit_logger_service_1.default.query({
        patient_id: 'patient_456',
        limit: 1,
        privacy_mode: complete_audit_logger_service_1.PrivacyMode.MINIMAL,
    });
    if (results.length > 0) {
        const entry = results[0];
 ...
 */
async function exampleFilterByModel() {
    console.log('Example 10: Filter by Model');
    console.log('-'.repeat(80));
    const results = await complete_audit_logger_service_1.default.query({
        model: 'gpt-4',
    });
    console.log(`  Queries using gpt-4: ${results.length}\n`);
    console.log('  ✅ Success\n');
}
/**
...
async function exampleExportJSON() {
    ...
    console.log('-'.repeat(80));
    const json = await complete_audit_logger_service_1.default.export({
        patient_id: 'patient_456',
        limit: 2,
    }, 'json');
...
async function exampleExportCSV() {
    ...
    console.log('-'.repeat(80));
    const csv = await complete_audit_logger_service_1.default.export({
        user_id: 'user_123',
        limit: 3,
    }, 'csv');
...
async function exampleGetStatistics() {
    ...
    console.log('-'.repeat(80));
    const stats = complete_audit_logger_service_1.default.getStatistics();
...
async function exampleLogFailedQuery() {
    ...
    await complete_audit_logger_service_1.default.logComplete(failedEntry);
...
async function exampleCompletePipeline() {
    ...
    console.log('    ✓ LLM generation (gpt-4-0613)');
...
async function exampleLogFilePath() {
    ...
    console.log('-'.repeat(80));
    const logPath = complete_audit_logger_service_1.default.getLogFilePath();
    console.log('  Audit log file:');
    console.log(`    Path: ${logPath}`);
    console.log(`    Format: Append-only JSON lines`);
    console.log(`    Privacy mode: ${complete_audit_logger_service_1.default.getPrivacyMode()}\n`);
    console.log('  ✅ Success\n');
}
/**
...
async function exampleExplain() {
    ...
    console.log('-'.repeat(80));
    const explanation = complete_audit_logger_service_1.default.explain();
    console.log('\n' + explanation.split('\n').map(line => `  ${line}`).join('\n'));
    console.log('\n  ✅ Success\n');
}
/**
...
async function runAllExamples() {
    ...
    try {
        ...
        complete_audit_logger_service_1.default.clear();
...
// runAllExamples();
//# sourceMappingURL=complete-audit-logger.example.js.map