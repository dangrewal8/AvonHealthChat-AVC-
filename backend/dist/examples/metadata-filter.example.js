...
exports.exampleFilterByPatient = exampleFilterByPatient;
exports.exampleFilterByType = exampleFilterByType;
exports.exampleFilterByDate = exampleFilterByDate;
exports.exampleFilterByAuthor = exampleFilterByAuthor;
exports.exampleCombinedFilters = exampleCombinedFilters;
exports.exampleFilterWithStats = exampleFilterWithStats;
exports.exampleIndexedFiltering = exampleIndexedFiltering;
exports.exampleVectorStoreFilter = exampleVectorStoreFilter;
exports.exampleFAISSPreFiltering = exampleFAISSPreFiltering;
exports.exampleFilterValidation = exampleFilterValidation;
exports.exampleFilterStats = exampleFilterStats;
exports.exampleBatchFiltering = exampleBatchFiltering;
exports.exampleEdgeCases = exampleEdgeCases;
exports.examplePerformanceComparison = examplePerformanceComparison;
exports.runAllExamples = runAllExamples;
const metadata_filter_service_1 = __importDefault(require("../services/metadata-filter.service"));
/**
...
function exampleFilterByType() {
    ...
    const filtered = metadata_filter_service_1.default.filterByType(chunks, types);
    console.log(`  Results: ${filtered.length} chunks\n`);
    filtered.forEach((chunk, i) => {
...
function exampleFilterByDate() {
    ...
    const filtered = metadata_filter_service_1.default.filterByDate(chunks, from, to);
    console.log(`  Results: ${filtered.length} chunks\n`);
    filtered.forEach((chunk, i) => {
        console.log(`  ${i + 1}. ${chunk.chunk_id}`);
        console.log(`     Date: ${chunk.metadata.date}`);
 ...
 */
function exampleFilterByAuthor() {
    console.log('Example 4: Filter by Author');
    console.log('-'.repeat(80));
    const chunks = generateSampleChunks();
    const author = 'Dr. Smith';
    console.log(`  Filtering by author: ${author}`);
    const filtered = metadata_filter_service_1.default.filterByAuthor(chunks, author);
    console.log(`  Results: ${filtered.length} chunks\n`);
    filtered.forEach((chunk, i) => {
        console.log(`  ${i + 1}. ${chunk.chunk_id} - ${chunk.metadata.artifact_type}`);
...
function exampleCombinedFilters() {
    ...
    console.log('');
    const filtered = metadata_filter_service_1.default.applyFilters(chunks, filters);
    console.log(`  Results: ${filtered.length} chunks\n`);
    filtered.forEach((chunk, i) => {
        console.log(`  ${i + 1}. ${chunk.chunk_id}`);
        console.log(`     Type: ${chunk.metadata.artifact_type}`);
 ...
 */
function exampleFilterWithStats() {
    ...
    const result = metadata_filter_service_1.default.applyFiltersWithStats(chunks, filters);
    ...
    console.log(`    Reduction: ${(((result.total_before - result.total_after) / result.total_before) * 100).toFixed(1)}%`);
    console.log('');
    console.log('  ✅ Success\n');
}
/**
 ...
 */
function exampleIndexedFiltering() {
    ...
    const stats = metadata_filter_service_1.default.getIndexStats();
 ...
 */
function exampleVectorStoreFilter() {
    ...
    const vectorStoreFilter = metadata_filter_service_1.default.getVectorStoreFilter(filters);
 ...
 */
function exampleFAISSPreFiltering() {
    ...
    console.log(`    Types: ${filters.artifact_types?.join(', ')}`);
    console.log('');
    const chunkIds = metadata_filter_service_1.default.getFilteredChunkIds(filters);
 ...
 */
function exampleFilterValidation() {
    ...
    testFilters.forEach(({ name, filters }, i) => {
        ...
        const validation = metadata_filter_service_1.default.validateFilters(filters);
...
function exampleFilterStats() {
    ...
    const stats = metadata_filter_service_1.default.getFilterStats(filters);
    console.log('  Filter Statistics:');
    console.log(`    Total chunks: ${stats.total_chunks}`);
 ...
 */
function exampleBatchFiltering() {
    ...
    const results = metadata_filter_service_1.default.batchFilter(chunks, filtersList);
...
function exampleEdgeCases() {
    ...
    const emptyFilters = {};
    const result1 = metadata_filter_service_1.default.applyFilters(chunks, emptyFilters);
    ...
    const result2 = metadata_filter_service_1.default.applyFilters(chunks, noMatchFilters);
    console.log(`    Input: ${chunks.length} chunks`);
    console.log(`    Output: ${result2.length} chunks (empty array)`);
    console.log('');
    console.log('  Edge Case 3: Empty chunk array');
    const result3 = metadata_filter_service_1.default.applyFilters([], {
        patient_id: 'patient_123',
    });
    ...
    const result4 = metadata_filter_service_1.default.applyFilters(chunks, dateFromOnly);
...
function examplePerformanceComparison() {
    ...
    const result1 = metadata_filter_service_1.default.applyFilters(largeDataset, filters);
    const end1 = Date.now();
    ...
    metadata_filter_service_1.default.buildIndexes(largeDataset);
    const start2 = Date.now();
    const result2 = metadata_filter_service_1.default.applyFiltersWithIndexes(filters);
    const end2 = Date.now();
...
// runAllExamples();
//# sourceMappingURL=metadata-filter.example.js.map