...
exports.exampleRetrievalSuccessGenerationTimeout = exampleRetrievalSuccessGenerationTimeout;
exports.exampleOnlyQueryUnderstanding = exampleOnlyQueryUnderstanding;
exports.exampleNothingSucceeded = exampleNothingSucceeded;
exports.exampleExtractionSuccessGenerationTimeout = exampleExtractionSuccessGenerationTimeout;
exports.exampleOrchestratorIntegration = exampleOrchestratorIntegration;
exports.exampleCompletionPercentage = exampleCompletionPercentage;
exports.exampleHasPartialResults = exampleHasPartialResults;
exports.exampleUserMessages = exampleUserMessages;
exports.exampleFallbackDetermination = exampleFallbackDetermination;
exports.exampleRealWorldTimeout = exampleRealWorldTimeout;
exports.exampleExplain = exampleExplain;
exports.runAllExamples = runAllExamples;
const partial_results_handler_service_1 = __importDefault(require("../services/partial-results-handler.service"));
/**
 * Example 1: Retrieval succeeded, generation timed out
 */
function exampleRetrievalSuccessGenerationTimeout() {
    ...
    const partial = partial_results_handler_service_1.default.handlePartialResult(error, context);
    ...
    console.log();
    // Format for UI
    const uiResponse = partial_results_handler_service_1.default.formatPartialResponse(partial);
 ...
 */
function exampleOnlyQueryUnderstanding() {
 ...
 */
function exampleNothingSucceeded() {
 ...
 */
function exampleExtractionSuccessGenerationTimeout() {
    ...
    // Determine fallback
    const fallback = partial_results_handler_service_1.default.determineFallback(partial.completed_stages, context);
    console.log('\n  Fallback Strategy:');
...
async function exampleOrchestratorIntegration() {
    ...
    async function processQueryWithTimeout(_query, patientId) {
        const context = {
            stage: 'query_understanding',
            data: {},
            start_time: Date.now(),
        ...
        try {
            // Set timeout
            const timeoutPromise = new Promise((_, reject) => setTimeout(() => reject(new Error('TIMEOUT')), context.timeout_ms));
            // Work promise
            ...
            if (error.message === 'TIMEOUT') {
                ...
                const partial = partial_results_handler_service_1.default.handlePartialResult(error, context);
                return partial_results_handler_service_1.default.formatPartialResponse(partial);
            }
            throw error;
        }
    }
 ...
 */
function exampleCompletionPercentage() {
    ...
    scenarios.forEach(scenario => {
        const percentage = partial_results_handler_service_1.default.getCompletionPercentage(scenario.stages);
 ...
 */
function exampleHasPartialResults() {
    console.log('Example 7: Check if Partial Results Available');
    console.log('-'.repeat(80));
    const scenarios = [
        {
            name: 'No data',
            context: { stage: 'query_understanding', data: {}, start_time: Date.now(), timeout_ms: 6000 },
            expected: false,
        },
        {
            name: 'Query only',
    ...
    scenarios.forEach(scenario => {
        const hasPartial = partial_results_handler_service_1.default.hasPartialResults(scenario.context);
...
function exampleUserMessages() {
    ...
    stages.forEach(({ stage, completedStages }) => {
        const message = partial_results_handler_service_1.default.createPartialMessage(stage, completedStages);
        console.log(`    ${stage}:`);
        console.log(`      "${message}"`);
        console.log();
    });
 ...
 */
function exampleFallbackDetermination() {
    ...
    scenarios.forEach((completedStages, i) => {
        const fallback = partial_results_handler_service_1.default.determineFallback(completedStages, context);
 ...
 */
async function exampleRealWorldTimeout() {
    console.log('Example 10: Real-World Timeout Scenario');
    console.log('-'.repeat(80));
    console.log('  Scenario: OpenAI generation takes too long\n');
    async function ragPipelineWithTimeout(_query) {
        const context = {
            stage: 'query_understanding',
            data: {},
            start_time: Date.now(),
        ...
        try {
            ...
            if (error.message === 'TIMEOUT') {
                ...
                const partial = partial_results_handler_service_1.default.handlePartialResult(error, context);
                const response = partial_results_handler_service_1.default.formatPartialResponse(partial);
...
function exampleExplain() {
    ...
    console.log('-'.repeat(80));
    const explanation = partial_results_handler_service_1.default.explain();
    console.log('\n' + explanation.split('\n').map(line => `  ${line}`).join('\n'));
    console.log('\n  ✅ Success\n');
}
/**
...
// runAllExamples();
//# sourceMappingURL=partial-results-handler.example.js.map