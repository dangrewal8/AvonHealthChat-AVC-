...
exports.exampleBasicQuery = exampleBasicQuery;
exports.exampleSuccessfulPipeline = exampleSuccessfulPipeline;
exports.exampleCustomTimeout = exampleCustomTimeout;
exports.exampleDisableAuditLogging = exampleDisableAuditLogging;
exports.exampleSessionTracking = exampleSessionTracking;
exports.examplePerformanceMonitoring = examplePerformanceMonitoring;
exports.exampleResponseStructure = exampleResponseStructure;
exports.exampleConfidenceScoring = exampleConfidenceScoring;
exports.exampleProvenanceTracking = exampleProvenanceTracking;
exports.exampleStructuredExtractions = exampleStructuredExtractions;
exports.exampleMedicalRecordQuery = exampleMedicalRecordQuery;
exports.exampleDifferentIntents = exampleDifferentIntents;
exports.examplePipelineStages = examplePipelineStages;
exports.exampleRealWorldScenario = exampleRealWorldScenario;
exports.exampleExplain = exampleExplain;
exports.runAllExamples = runAllExamples;
const orchestrator_service_1 = __importDefault(require("../services/orchestrator.service"));
/**
...
async function exampleBasicQuery() {
    ...
    try {
        const response = await orchestrator_service_1.default.processQuery(query, patientId);
 ...
 */
async function exampleSuccessfulPipeline() {
 ...
 */
async function exampleCustomTimeout() {
 ...
 */
async function exampleDisableAuditLogging() {
 ...
 */
async function exampleSessionTracking() {
...
async function examplePerformanceMonitoring() {
    ...
    try {
        ...
        if (response.metadata?.stages) {
            ...
            console.log('    Query Understanding:', stages.query_understanding, 'ms', `(${((stages.query_understanding / total) * 100).toFixed(1)}%)`);
            console.log('    Retrieval:', stages.retrieval, 'ms', `(${((stages.retrieval / total) * 100).toFixed(1)}%)`);
            console.log('    Generation:', stages.generation, 'ms', `(${((stages.generation / total) * 100).toFixed(1)}%)`);
            console.log('    Confidence Scoring:', stages.confidence_scoring, 'ms', `(${((stages.confidence_scoring / total) * 100).toFixed(1)}%)`);
            console.log('    Provenance Formatting:', stages.provenance_formatting, 'ms', `(${((stages.provenance_formatting / total) * 100).toFixed(1)}%)`);
            console.log('    Response Building:', stages.response_building, 'ms', `(${((stages.response_building / total) * 100).toFixed(1)}%)`);
            console.log('    Audit Logging:', stages.audit_logging, 'ms', `(${((stages.audit_logging / total) * 100).toFixed(1)}%)`);
        }
 ...
 */
async function exampleResponseStructure() {
 ...
 */
async function exampleConfidenceScoring() {
 ...
 */
async function exampleStructuredExtractions() {
    ...
    try {
        ...
        response.structuredExtractions?.forEach((ext, i) => {
            console.log(`\n    Extraction ${i + 1}:`);
 ...
 */
async function exampleMedicalRecordQuery() {
...
async function exampleDifferentIntents() {
    ...
    for (const { query, intent } of queries) {
        try {
            const response = await orchestrator_service_1.default.processQuery(query, 'patient_909');
            console.log(`\n  Query: "${query}"`);
 ...
 */
async function examplePipelineStages() {
...
async function exampleExplain() {
    ...
    console.log('-'.repeat(80));
    const explanation = orchestrator_service_1.default.explain();
    console.log('\n' + explanation.split('\n').map(line => `  ${line}`).join('\n'));
    console.log('\n  ✅ Success\n');
}
/**
...
// runAllExamples();
//# sourceMappingURL=orchestrator.example.js.map