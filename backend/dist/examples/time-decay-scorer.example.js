...
exports.exampleBasicTimeDecay = exampleBasicTimeDecay;
exports.exampleDecayFactorCalculation = exampleDecayFactorCalculation;
exports.exampleDecayMilestones = exampleDecayMilestones;
exports.exampleDecayCurveVisualization = exampleDecayCurveVisualization;
exports.exampleDecayCurveData = exampleDecayCurveData;
exports.exampleBeforeAfterComparison = exampleBeforeAfterComparison;
exports.exampleDecayAnalysis = exampleDecayAnalysis;
exports.exampleExplainDecay = exampleExplainDecay;
exports.exampleRankChanges = exampleRankChanges;
exports.exampleMostAffected = exampleMostAffected;
exports.exampleBatchTimeDecay = exampleBatchTimeDecay;
exports.exampleDecayRate = exampleDecayRate;
exports.runAllExamples = runAllExamples;
const time_decay_scorer_service_1 = __importDefault(require("../services/time-decay-scorer.service"));
/**
 ...
 */
function exampleBasicTimeDecay() {
    ...
    candidates.forEach((c) => {
        const daysAgo = time_decay_scorer_service_1.default.calculateDaysAgo(c.metadata.date);
        console.log(`    ${c.rank}. ${c.chunk.chunk_id} - ${daysAgo} days ago`);
    });
    console.log('\n  Applying time decay...\n');
    const decayed = time_decay_scorer_service_1.default.applyTimeDecay(candidates);
 ...
 */
function exampleDecayFactorCalculation() {
    ...
    testDates.forEach(({ label, days }) => {
        ...
        const decayFactor = time_decay_scorer_service_1.default.calculateDecayFactor(testDate.toISOString());
 ...
 */
function exampleDecayMilestones() {
    console.log('Example 3: Decay Milestones');
    console.log('-'.repeat(80));
    const milestones = time_decay_scorer_service_1.default.getDecayMilestones();
    ...
    milestones.forEach((m) => {
        ...
        if (m.penalty_pct < 10)
            effect = 'No penalty';
        else if (m.penalty_pct < 30)
            effect = '7% penalty';
        else if (m.penalty_pct < 50)
            effect = '26% penalty';
        else if (m.penalty_pct < 70)
            effect = '59% penalty';
        else if (m.penalty_pct < 90)
            effect = '83% penalty';
        else
            effect = '97% penalty';
        console.log(`  ${String(m.days_ago).padStart(8)} │ ${m.decay_factor.toFixed(4).padStart(12)} │ ${m.penalty_pct.toFixed(0).padStart(7)}% │ ${effect}`);
    });
    console.log('\n  ✅ Success\n');
}
/**
 ...
 */
function exampleDecayCurveVisualization() {
    console.log('Example 4: Decay Curve Visualization');
    console.log('-'.repeat(80));
    console.log('\n' + time_decay_scorer_service_1.default.visualizeDecayCurve(50) + '\n');
    console.log('  ✅ Success\n');
}
/**
 * Example 5: Decay curve data points
 */
function exampleDecayCurveData() {
    console.log('Example 5: Decay Curve Data Points');
    console.log('-'.repeat(80));
    const curve = time_decay_scorer_service_1.default.getDecayCurve(365, 30);
...
function exampleBeforeAfterComparison() {
    ...
    const candidates = generateSampleCandidates();
    const decayed = time_decay_scorer_service_1.default.applyTimeDecay(candidates);
    const comparison = time_decay_scorer_service_1.default.compareBeforeAfter(candidates, decayed);
 ...
 */
function exampleDecayAnalysis() {
    console.log('Example 7: Decay Analysis for Specific Date');
    console.log('-'.repeat(80));
    const now = new Date();
    const testDate = new Date(now);
    testDate.setDate(testDate.getDate() - 60); // 60 days ago
    const analysis = time_decay_scorer_service_1.default.analyzeDecay(testDate.toISOString());
...
function exampleExplainDecay() {
    ...
    if (candidate) {
        console.log('\n' + time_decay_scorer_service_1.default.explainDecay(candidate) + '\n');
    }
    console.log('  ✅ Success\n');
}
/**
...
function exampleRankChanges() {
    ...
    candidates.forEach((c) => {
        ...
        console.log(`       Age: ${time_decay_scorer_service_1.default.calculateDaysAgo(c.metadata.date)} days`);
        console.log('');
    });
    const decayed = time_decay_scorer_service_1.default.applyTimeDecay(candidates);
    console.log('  After Time Decay:\n');
 ...
 */
function exampleMostAffected() {
    console.log('Example 10: Most Affected Candidates');
    console.log('-'.repeat(80));
    const candidates = generateSampleCandidates();
    const decayed = time_decay_scorer_service_1.default.applyTimeDecay(candidates);
    // Find candidates with >50% penalty
    const mostAffected = time_decay_scorer_service_1.default.findMostAffected(decayed, 50);
 ...
 */
function exampleBatchTimeDecay() {
    ...
    const decayedLists = time_decay_scorer_service_1.default.batchApplyTimeDecay(candidateLists);
    decayedLists.forEach((decayed, i) => {
        const avgDecay = decayed.reduce((sum, c) => sum + c.time_decay_factor, 0) / decayed.length;
 ...
 */
function exampleDecayRate() {
    console.log('Example 12: Decay Rate Constant');
    console.log('-'.repeat(80));
    const decayRate = time_decay_scorer_service_1.default.getDecayRate();
...
// runAllExamples();
//# sourceMappingURL=time-decay-scorer.example.js.map