...
exports.calculateExtractionF1 = calculateExtractionF1;
exports.calculateCitationAccuracy = calculateCitationAccuracy;
exports.evaluateQueryResult = evaluateQueryResult;
exports.evaluateDataset = evaluateDataset;
exports.exportEvaluationReport = exportEvaluationReport;
exports.printEvaluationSummary = printEvaluationSummary;
/**
 * Calculate recall@k
 ...
 */
function calculatePrecision(retrievedIds, relevantIds, k = 5) {
    const topK = retrievedIds.slice(0, k);
    if (topK.length === 0) {
 ...
 */
function calculateExtractionF1(actualExtractions, expectedExtractions) {
    ...
    for (const expected of expectedExtractions) {
        const found = actualExtractions.some((actual) => extractionsMatch(actual, expected));
        if (found)
            matches++;
    }
 ...
 */
function extractionsMatch(actual, expected) {
    // Match on type
    ...
    switch (expected.type) {
        case 'medication_recommendation':
            return (actual.medication?.toLowerCase() ===
 ...
 */
function calculateCitationAccuracy(actualCitations, relevantIds) {
    if (actualCitations.length === 0) {
        return relevantIds.length === 0 ? 1.0 : 0.0;
    }
    const correctCitations = actualCitations.filter((citation) => relevantIds.includes(citation.artifact_id));
    return correctCitations.length / actualCitations.length;
}
/**
 * Evaluate a single query result against golden dataset entry
 */
function evaluateQueryResult(entry, result, retrievedArtifactIds) {
    const startTime = Date.now();
    // Retrieval metrics
    const recall_at_5 = calculateRecall(retrievedArtifactIds, entry.ground_truth.relevant_artifact_ids, 5);
    const precision_at_5 = calculatePrecision(retrievedArtifactIds, entry.ground_truth.relevant_artifact_ids, 5);
    const retrievedRelevant = retrievedArtifactIds
        .slice(0, 5)
        .filter((id) => entry.ground_truth.relevant_artifact_ids.includes(id));
    const missedRelevant = entry.ground_truth.relevant_artifact_ids.filter((id) => !retrievedArtifactIds.slice(0, 5).includes(id));
    const falsePositives = retrievedArtifactIds
        .slice(0, 5)
        .filter((id) => !entry.ground_truth.relevant_artifact_ids.includes(id));
    // Extraction metrics
    // Note: In real implementation, extract structured data from result
    const actualExtractions = []; // Would extract from result
    const { f1, precision, recall } = calculateExtractionF1(actualExtractions, entry.ground_truth.expected_extractions);
    // Citation metrics
    const citationAccuracy = calculateCitationAccuracy(result.provenance || [], entry.ground_truth.relevant_artifact_ids);
    const correctCitations = (result.provenance || []).filter((p) => entry.ground_truth.relevant_artifact_ids.includes(p.artifact_id)).length;
    const incorrectCitations = (result.provenance || []).length - correctCitations;
    const missingCitations = Math.max(0, entry.ground_truth.expected_sources_min - correctCitations);
    // Check acceptance criteria
    const failedCriteria = [];
    if (recall_at_5 < entry.acceptance_criteria.recall_at_5_min) {
        failedCriteria.push(`Recall@5: ${recall_at_5.toFixed(2)} < ${entry.acceptance_criteria.recall_at_5_min}`);
    }
    if (precision_at_5 < entry.acceptance_criteria.precision_at_5_min) {
        failedCriteria.push(`Precision@5: ${precision_at_5.toFixed(2)} < ${entry.acceptance_criteria.precision_at_5_min}`);
    }
    if (f1 < entry.acceptance_criteria.extraction_f1_min) {
        failedCriteria.push(`Extraction F1: ${f1.toFixed(2)} < ${entry.acceptance_criteria.extraction_f1_min}`);
    }
    if (citationAccuracy < entry.acceptance_criteria.citation_accuracy_min) {
        failedCriteria.push(`Citation Accuracy: ${citationAccuracy.toFixed(2)} < ${entry.acceptance_criteria.citation_accuracy_min}`);
    }
    if (result.confidence < entry.ground_truth.expected_confidence_min) {
        failedCriteria.push(`Confidence: ${result.confidence.toFixed(2)} < ${entry.ground_truth.expected_confidence_min}`);
    }
    ...
    return {
        ...
        extraction: {
            ...
            precision,
            recall,
            matched_extractions: Math.round(recall * entry.ground_truth.expected_extractions.length),
 ...
 */
async function evaluateDataset(dataset, queryExecutor) {
    const results = [];
    let passedCount = 0;
    // Evaluate each entry
    for (const entry of dataset.entries) {
        try {
            const { result, retrievedIds } = await queryExecutor(entry.query, entry.patient_id);
    ...
    const meanRecall = results.reduce((sum, r) => sum + r.retrieval.recall_at_5, 0) /
        totalResults;
    const meanPrecision = results.reduce((sum, r) => sum + r.retrieval.precision_at_5, 0) /
        totalResults;
    const meanF1 = results.reduce((sum, r) => sum + r.extraction.f1_score, 0) / totalResults;
    const meanCitationAccuracy = results.reduce((sum, r) => sum + r.citation.accuracy, 0) / totalResults;
    const meanConfidence = results.reduce((sum, r) => sum + r.confidence, 0) / totalResults;
    const meanExecutionTime = results.reduce((sum, r) => sum + r.execution_time_ms, 0) / totalResults;
 ...
 */
function exportEvaluationReport(report, pretty = true) {
    return pretty ? JSON.stringify(report, null, 2) : JSON.stringify(report);
}
/**
 * Print evaluation summary to console
 */
function printEvaluationSummary(report) {
    console.log('\n' + '='.repeat(80));
...
//# sourceMappingURL=golden-dataset-evaluator.js.map