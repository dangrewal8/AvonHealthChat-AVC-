...
if (result.error) {
    ...
    dotenv_1.default.config(); // Fall back to default .env
}
else {
...
class ConfigValidator {
    errors = [];
    warnings = [];
    getRequiredEnv(key, allowEmpty = false) {
    ...
    getRequiredEnvNumber(key, defaultValue) {
        const value = process.env[key];
        if (!value) {
    ...
    getOptionalEnvNumber(key, defaultValue) {
        const value = process.env[key];
        if (!value) {
            return defaultValue;
        }
    ...
    getRequiredEnvBoolean(key, defaultValue) {
        const value = process.env[key];
        if (!value) {
            return defaultValue;
        }
    ...
    validate() {
        ...
        if (embeddingProvider !== 'ollama') {
            this.errors.push(`SECURITY VIOLATION: EMBEDDING_PROVIDER must be 'ollama' for HIPAA compliance. ` +
                `Got: '${embeddingProvider}'. External providers are not permitted.`);
        }
        if (llmProvider !== 'ollama') {
            this.errors.push(`SECURITY VIOLATION: LLM_PROVIDER must be 'ollama' for HIPAA compliance. ` +
        ...
        const config = {
            ...
            vectorDb: {
                type: this.getOptionalEnv('VECTOR_DB_TYPE', 'chromadb'),
            },
            faiss: {
            ...
            logging: {
                level: this.getOptionalEnv('LOG_LEVEL', this.getOptionalEnv('NODE_ENV', 'development') === 'production' ? 'info' : 'debug'),
            },
            security: {
                httpsRedirect: this.getRequiredEnvBoolean('HTTPS_REDIRECT', this.getOptionalEnv('NODE_ENV', 'development') === 'production'),
        ...
        if (this.errors.length > 0) {
            ...
            throw new Error(`Configuration validation failed with ${this.errors.length} error(s). Check your .env file.`);
        }
    ...
    validateConfig(config) {
        ...
        if (!['chromadb', 'faiss'].includes(config.vectorDb.type)) {
            throw new Error(`Invalid VECTOR_DB_TYPE: ${config.vectorDb.type}. Must be 'chromadb' or 'faiss'`);
        }
