 ...
 */
function generateFallbackShortAnswer(query, queryIntent, data) {
    const { patient, care_plans, medications, notes, allergies, conditions, vitals, family_history, appointments, documents, form_responses, insurance_policies } = data;
     ...
     */
    const dataNotAvailable = (dataType, suggestion) => {
     ...
     */
    const detectMultiPartQuestion = (q) => {
        ...
        if (lower.includes(' and ')) {
            ...
            const questionParts = parts.filter(part => questionWords.some(qw => part.trim().startsWith(qw)));
    ...
    if ((queryLower.includes('taking for') || queryLower.includes('prescribed for')) && medications) {
        ...
        if (conditionMatch && conditionMatch[1]) {
            ...
            const relatedMeds = activeMeds.filter((m) => m.name?.toLowerCase().includes(condition) ||
                care_plans?.some(cp => cp.name?.toLowerCase().includes(condition)) // API uses 'name', not 'title'
            );
            if (relatedMeds.length > 0) {
                const medNames = relatedMeds.map((m) => m.name).join(', ');
    ...
    if ((queryLower.includes('who prescribed') || queryLower.includes('which doctor') ||
        ...
        const prescribers = [...new Set(medications.map((m) => m.created_by).filter(Boolean))];
    ...
    if (queryLower.match(/(?:current|active|present).+(?:medication|med).+(?:and|,).+(?:past|previous|historical|inactive).+(?:medication|med)/i) ||
        ...
        if (medications && medications.length > 0) {
            ...
            if (activeMeds.length > 0) {
                const activeNames = activeMeds.slice(0, 5).map((m) => formatMedicationName(m)).join(', ');
            ...
            if (inactiveMeds.length > 0) {
                const inactiveNames = inactiveMeds.slice(0, 5).map((m) => formatMedicationName(m)).join(', ');
    ...
    if ((queryLower.includes('past') || queryLower.includes('previous') || queryLower.includes('historical') ||
        ...
        if (medications && medications.length > 0) {
            ...
            if (inactiveMeds.length > 0) {
                ...
                const medNames = inactiveMeds.slice(0, 5).map((m) => {
    ...
    if (isConditionQuery) {
        ...
        if (conditions && conditions.length > 0) {
            ...
            const conditionNames = conditions.slice(0, 3).map((c) => c.description || c.name || 'Unnamed condition').join(', ');
    ...
    if ((queryLower.includes('does') || queryLower.includes('is there')) &&
        ...
        if (care_plans && care_plans.length > 0) {
            ...
            const conditionIndex = Math.max(words.indexOf('have') + 1, words.indexOf('for') + 1, words.indexOf('plan') + 1);
            if (conditionIndex > 0 && conditionIndex < words.length) {
                const potentialCondition = words.slice(conditionIndex).join(' ').replace(/[?.,]/g, '');
                const matchingPlan = care_plans.find((cp) => cp.name?.toLowerCase().includes(potentialCondition.toLowerCase()));
                if (matchingPlan) {
    ...
    if ((queryLower.includes('status') || queryLower.includes('active')) && care_plans) {
        if (care_plans.length > 0) {
            const planNames = care_plans.map((cp) => cp.name).join(', ');
    ...
    if ((queryLower.includes('what did') || queryLower.includes('what was')) &&
        ...
        if (notes && notes.length > 0) {
            ...
            if (recentNote.sections && Array.isArray(recentNote.sections)) {
                const parts = [];
                recentNote.sections.forEach((section) => {
    ...
    if ((queryLower.includes('who is') || queryLower.includes('who are')) &&
        ...
        notes?.forEach((n) => n.created_by && providers.add(n.created_by));
        care_plans?.forEach((cp) => {
            if (cp.created_by)
                providers.add(cp.created_by);
            if (cp.assigned_to && cp.assigned_to !== 'user_null')
                providers.add(cp.assigned_to);
        });
        medications?.forEach((m) => m.created_by && providers.add(m.created_by));
        if (providers.size > 0) {
    ...
    if (queryIntent.primary === 'summary' || queryLower.includes('summary') ||
        ...
        if (hasData) {
            ...
            if (planCount > 0)
                parts.push(`${planCount} care plan${planCount !== 1 ? 's' : ''}`);
            if (noteCount > 0)
                parts.push(`${noteCount} clinical note${noteCount !== 1 ? 's' : ''}`);
    ...
    if (queryIntent.primary === 'patient_contact' ||
        ...
        if (patient) {
            const contactParts = [];
            if (patient.email)
                contactParts.push(`Email: ${patient.email}`);
            if (patient.phone)
                contactParts.push(`Phone: ${patient.phone}`);
            if (patient.alternate_phone)
                contactParts.push(`Alternate: ${patient.alternate_phone}`);
    ...
    if (queryIntent.primary === 'patient_gender' ||
        ...
        if (patient) {
            const genderInfo = [];
            if (patient.gender)
                genderInfo.push(`Gender: ${patient.gender}`);
            if (patient.sex && patient.sex !== patient.gender)
                genderInfo.push(`Sex: ${patient.sex}`);
            if (patient.pronouns)
                genderInfo.push(`Pronouns: ${patient.pronouns}`);
    ...
    if ((queryLower.includes('address') || queryLower.includes('where') && (queryLower.includes('live') || queryLower.includes('from'))) &&
        ...
        if (patient && patient.addresses && patient.addresses.length > 0) {
            ...
            if (primaryAddress.street)
                parts.push(primaryAddress.street);
            if (primaryAddress.street_line_2)
                parts.push(primaryAddress.street_line_2);
            ...
            if (primaryAddress.postal_code)
                parts.push(primaryAddress.postal_code);
            if (parts.length > 0) {
    ...
    if ((queryLower.includes('race') || queryLower.includes('ethnicity')) && queryLower.includes('patient')) {
        if (patient) {
            ...
            if (patient.race)
                demographicInfo.push(`Race: ${patient.race}`);
            if (patient.ethnicity)
                demographicInfo.push(`Ethnicity: ${patient.ethnicity}`);
        ...
        if (medications) {
            for (const med of medications) {
                if (med.name && med.active) {
                    ...
                    const hasMatch = medNameWords.some((medWord) => words.includes(medWord.replace(/[^a-z0-9]/g, '')));
                    if (hasMatch) {
                        matchedMed = med;
                        break;
                    }
            ...
            if (matchedMed) {
                ...
                if (care_plans) {
                    for (const cp of care_plans) {
                        ...
                        if (medNameLower.includes('metformin') && cpNameLower.includes('diabetes'))
                            relatedCondition = cp.name;
                        else if (medNameLower.includes('lisinopril') && cpNameLower.includes('hypertension'))
                            relatedCondition = cp.name;
                        else if (medNameLower.includes('atorvastatin') && cpNameLower.includes('cholesterol'))
                            relatedCondition = cp.name;
                        else if (medNameLower.includes('levothyroxine') && cpNameLower.includes('thyroid'))
                            relatedCondition = cp.name;
                        else if (medNameLower.includes('insulin') && cpNameLower.includes('diabetes'))
                            relatedCondition = cp.name;
                        else if (medNameLower.includes('albuterol') && cpNameLower.includes('asthma'))
                            relatedCondition = cp.name;
                        if (relatedCondition)
                            break;
                    }
    ...
    if ((queryLower.includes('does') || queryLower.includes('is')) &&
        ...
        if (mentionedCondition && care_plans) {
            const matchingPlan = care_plans.find(cp => cp.name?.toLowerCase().includes(mentionedCondition));
            if (matchingPlan) {
    ...
    if (queryLower.match(/(?:active|current|present).+(?:allerg).+(?:and|,).+(?:inactive|past|previous|historical).+(?:allerg)/i) ||
        ...
        if (allergies && allergies.length > 0) {
            ...
            if (activeAllergies.length > 0) {
                const activeNames = activeAllergies.slice(0, 5).map((a) => a.name).join(', ');
            ...
            if (inactiveAllergies.length > 0) {
                const inactiveNames = inactiveAllergies.slice(0, 5).map((a) => a.name).join(', ');
    ...
    if ((queryLower.includes('allerg') || queryLower.includes('allergic')) &&
        ...
        if (allergies && allergies.length > 0) {
            ...
            const words = queryLower.split(/\s+/);
            const matchedAllergy = activeAllergies.find((a) => words.some(word => a.name?.toLowerCase().includes(word)));
            if (matchedAllergy) {
                const details = [];
                details.push(`Yes, the patient is allergic to ${matchedAllergy.name}`);
                if (matchedAllergy.severity)
                    details.push(`Severity: ${matchedAllergy.severity}`);
                if (matchedAllergy.reaction)
                    details.push(`Reaction: ${matchedAllergy.reaction}`);
                if (matchedAllergy.onset_date)
                    details.push(`Since: ${formatDate(matchedAllergy.onset_date)}`);
                return details.join('. ') + '.';
            }
            // General allergy list
            if (activeAllergies.length > 0) {
                const allergyNames = activeAllergies.slice(0, 5).map((a) => a.name).join(', ');
    ...
    if (queryLower.includes('blood pressure') || queryLower.includes('temperature') ||
        ...
        if (vitals && vitals.length > 0) {
            // Sort by most recent
            const sortedVitals = [...vitals].sort((a, b) => new Date(b.created_at).getTime() - new Date(a.created_at).getTime());
            ...
            if (latest.blood_pressure)
                vitalParts.push(`BP: ${latest.blood_pressure}`);
            if (latest.pulse)
                vitalParts.push(`Pulse: ${latest.pulse}`);
            if (latest.temperature)
                vitalParts.push(`Temp: ${latest.temperature}`);
            if (latest.oxygen_saturation)
                vitalParts.push(`O2: ${latest.oxygen_saturation}`);
            if (latest.weight)
                vitalParts.push(`Weight: ${latest.weight}`);
    ...
    if (queryLower.includes('appointment') || queryLower.includes('next visit') ||
        ...
        if (appointments && appointments.length > 0) {
            ...
            const futureCount = appointments.filter((a) => new Date(a.start_time) > now).length;
            const pastCount = appointments.filter((a) => new Date(a.start_time) <= now).length;
    ...
    if ((queryLower.includes('family') && (queryLower.includes('history') || queryLower.includes('disease'))) ||
        ...
        if (family_history && family_history.length > 0) {
            ...
            if (queryLower.includes('mother') || queryLower.includes('father') ||
                ...
                for (const [rel, code] of Object.entries(relationshipMap)) {
                    if (queryLower.includes(rel)) {
                        const familyMember = family_history.find((fh) => fh.relationship === code);
                        if (familyMember && familyMember.diagnoses && familyMember.diagnoses.length > 0) {
                            const conditions = familyMember.diagnoses.map((d) => d.description || d.diagnosis).join(', ');
            ...
            const allDiagnoses = family_history.flatMap((fh) => fh.diagnoses || []);
            if (allDiagnoses.length > 0) {
                const uniqueConditions = [...new Set(allDiagnoses.map((d) => d.description || d.diagnosis))];
    ...
    if (queryLower.includes('insurance') || queryLower.includes('coverage') ||
        ...
        if (insurance_policies && insurance_policies.length > 0) {
            const primaryPolicy = insurance_policies.find((ip) => ip.type?.toLowerCase() === 'primary');
            if (primaryPolicy) {
...
function generateFallbackDetailedSummary(query, queryIntent, data) {
    ...
    const detectMultiPartForSummary = (q) => {
    ...
    function formatMedicationsSummary(meds) {
        ...
        if (activeMeds.length > 0) {
            ...
            activeMeds.slice(0, 10).forEach((med, idx) => {
                ...
                // Instructions
                if (med.sig)
                    output += `   Instructions: ${med.sig}\n`;
                // Timeline
                if (med.start_date)
                    output += `   Started: ${new Date(med.start_date).toLocaleDateString()}`;
                if (med.created_by)
                    output += ` (prescribed by ${med.created_by})`;
                if (med.start_date)
                    output += '\n';
                // Quantity and refills
                if (med.quantity || med.refills !== null) {
                    if (med.quantity)
                        output += `   Quantity: ${med.quantity}`;
                    if (med.refills !== null && med.refills !== undefined)
                        output += ` (${med.refills} refills)`;
                    output += '\n';
                }
                output += '\n';
            });
        ...
        if (inactiveMeds.length > 0 && activeMeds.length < 5) {
            ...
            inactiveMeds.slice(0, 5).forEach((med, idx) => {
                output += `${idx + 1}. ${med.name}`;
                if (med.dose_form)
                    output += ` ${med.dose_form}`;
                if (med.strength)
                    output += ` - ${med.strength}`;
                if (med.end_date)
                    output += ` (discontinued ${new Date(med.end_date).toLocaleDateString()})`;
                output += '\n';
            });
        }
        return output;
    ...
    function formatConditionsSummary(conditionsList) {
        ...
        conditionsList.slice(0, 5).forEach((condition, idx) => {
            ...
            if (condition.active !== undefined)
                output += condition.active ? ' [Active]' : ' [Inactive]';
            output += '\n';
            if (condition.onset_date)
                output += `   Onset: ${new Date(condition.onset_date).toLocaleDateString()}\n`;
            if (condition.end_date)
                output += `   Ended: ${new Date(condition.end_date).toLocaleDateString()}\n`;
            if (condition.comment)
                output += `   Note: ${condition.comment.substring(0, 150)}${condition.comment.length > 150 ? '...' : ''}\n`;
            output += '\n';
        });
        return output;
    }
    function formatCarePlansSummary(plans) {
        ...
        plans.slice(0, 5).forEach((cp, idx) => {
            output += `${idx + 1}. ${cp.name}\n`;
            if (cp.description)
                output += `   ${cp.description.substring(0, 150)}...\n`;
            if (cp.created_at)
                output += `   Created: ${new Date(cp.created_at).toLocaleDateString()}\n`;
            output += '\n';
        });
        return output;
    }
    function formatNotesSummary(notesList) {
        let output = `Total Clinical Notes: ${notesList.length}\n\n`;
        notesList.slice(0, 5).forEach((note, idx) => {
            output += `${idx + 1}. ${note.name || 'Clinical Note'}`;
            if (note.created_at)
                output += ` - ${new Date(note.created_at).toLocaleDateString()}`;
            output += '\n';
            if (note.created_by)
                output += `   Provider: ${note.created_by}\n`;
            output += '\n';
        });
        return output;
    }
    ...
    const formatMedication = (med, idx) => {
        ...
        const details = [];
        if (med.sig)
            details.push(med.sig); // Administration instructions
        if (med.start_date)
            details.push(`Started: ${formatDate(med.start_date)}`);
        if (med.active !== undefined)
            details.push(med.active ? 'Active' : 'Inactive');
        if (med.created_at)
            details.push(`Added: ${formatDate(med.created_at)}`);
        if (details.length > 0) {
            text += `   ${details.join(' | ')}\n`;
        }
        if (med.comment) {
    ...
    const formatCarePlan = (cp, idx) => {
        ...
        if (cp.start_date)
            details.push(`Started: ${formatDate(cp.start_date)}`);
        if (cp.created_at)
            details.push(`Created: ${formatDate(cp.created_at)}`);
        if (details.length > 0) {
            text += `   ${details.join(' | ')}\n`;
        }
        return text;
        ...
        note.sections.forEach((section) => {
            ...
            if (section.answers && Array.isArray(section.answers)) {
                section.answers.forEach((answer) => {
                    ...
                    if (answer.name)
                        contentParts.push(`  - ${answer.name}`);
                    if (answer.value)
                        contentParts.push(`  ${answer.value}`);
                    if (answer.text)
                        contentParts.push(`  ${answer.text}`);
                });
            }
        });
    ...
    if (isCondQuery) {
        ...
        if (conditions && conditions.length > 0) {
            ...
            conditions.forEach((condition, idx) => {
                ...
                summary += '\n';
                if (condition.active !== undefined)
                    summary += `   Status: ${condition.active ? 'Active' : 'Inactive'}\n`;
                if (condition.onset_date)
                    summary += `   Onset: ${formatDate(condition.onset_date)}\n`;
                if (condition.end_date)
                    summary += `   End Date: ${formatDate(condition.end_date)}\n`;
                if (condition.comment)
                    summary += `   Note: ${condition.comment}\n`;
                if (condition.created_by)
                    summary += `   Documented by: ${condition.created_by}\n`;
                summary += '\n';
            });
            return summary.trim();
        }
    ...
    if (care_plans && care_plans.length > 0) {
        summary += `CARE PLANS (${care_plans.length} total)\n`;
        care_plans.slice(0, 5).forEach((cp, idx) => {
            summary += formatCarePlan(cp, idx) + '\n';
        });
    ...
    if (medications && medications.length > 0) {
        ...
        if (activeMeds.length > 0) {
            ...
            activeMeds.slice(0, 8).forEach((med, idx) => {
                summary += formatMedication(med, idx) + '\n';
            });
...
router.post('/query', async (req, res) => {
    ...
    try {
        ...
        const queryAnalysis = (0, enhanced_query_understanding_1.analyzeQuery)(query);
            ...
            if (sourceType === 'medications' && medications) {
                medications.forEach((med) => {
                    ...
                    if (unitMatch) {
                        const units = parseInt(unitMatch[1]);
                        if (units >= 1000000)
                            displayStrength = `${(units / 1000000).toFixed(1).replace(/\.0$/, '')}m units`;
                        else if (units >= 1000)
                    ...
                    if (displayStrength)
                        snippetParts.push(displayStrength);
                    if (sig)
                        snippetParts.push(sig.substring(0, 60));
        ...
        const structured_extractions = [];
        // Helper: Format strength for display
        const formatStrengthShort = (strength) => {
            if (!strength)
                return '';
            const unitMatch = strength.match(/^(\d+)\s*UNIT$/i);
            if (unitMatch) {
            ...
            medications.forEach((med) => {
                if (med.name && detailed_summary.toLowerCase().includes(med.name.toLowerCase())) {
                    ...
                    if (strength)
                        parts.push(strength);
                    if (med.sig)
                        parts.push(`Take: ${med.sig.substring(0, 80)}${med.sig.length > 80 ? '...' : ''}`);
                    if (med.start_date)
                        parts.push(`Started: ${med.start_date}`);
                    parts.push(med.active ? 'Active' : 'Inactive');
                    if (med.created_by)
                        parts.push(`Added by: ${med.created_by}`);
        ...
        if (care_plans && detailed_summary) {
            care_plans.forEach((cp) => {
                if (cp.name && detailed_summary.toLowerCase().includes(cp.name.toLowerCase())) {
                    ...
                    const parts = [];
                    if (cp.description)
                        parts.push(cp.description.substring(0, 100) + (cp.description.length > 100 ? '...' : ''));
                    parts.push(cp.share_with_patient ? 'Shared with patient' : 'Not shared');
                    if (cp.created_at)
                        parts.push(`Created: ${cp.created_at.split('T')[0]}`);
                    if (cp.created_by)
                        parts.push(`By: ${cp.created_by}`);
                    structured_extractions.push({
                        type: 'condition',
                        value: cp.name,
                        relevance: 0.85,
 ...
 */
router.get('/queries/recent', async (_req, res) => {
    res.json({
        queries: [],
 ...
 */
router.get('/emr/care_plans', async (req, res) => {
    try {
        ...
        const carePlans = await avonHealthService.getCarePlans(patient_id);
        res.json(carePlans);
    }
    catch (error) {
...
router.get('/emr/medications', async (req, res) => {
    try {
        const { patient_id } = req.query;
        if (!patient_id) {
            res.status(400).json({ error: 'patient_id required' });
        ...
        const medications = await avonHealthService.getMedications(patient_id);
...
router.get('/emr/notes', async (req, res) => {
    try {
        const { patient_id } = req.query;
        if (!patient_id) {
            res.status(400).json({ error: 'patient_id required' });
        ...
        const notes = await avonHealthService.getNotes(patient_id);
        res.json(notes);
    }
    catch (error) {
        res.status(500).json({ error: error.message });
...
router.get('/emr/all', async (req, res) => {
    try {
        const { patient_id } = req.query;
        if (!patient_id) {
            res.status(400).json({ error: 'patient_id required' });
