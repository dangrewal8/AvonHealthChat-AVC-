...
class AvonHealthService {
    constructor(credentials) {
        ...
        this.patientDataCache = new Map();
        this.patientCacheTTL = 300000; // 5 minutes default
        this.maxPatientCacheSize = 100; // Max 100 patients cached
        this.patientCacheHits = 0;
        this.patientCacheMisses = 0;
        this.prefetchedPatients = new Set();
        ...
        this.artifactCache = new lru_cache_1.LRUCache({
     ...
     */
    async retryWithBackoff(operation, config = this.DEFAULT_RETRY_CONFIG, context = 'operation') {
        let lastError;
        for (let attempt = 1; attempt <= config.maxAttempts; attempt++) {
            try {
                ...
                return await Promise.race([
                    operation(),
                    new Promise((_, reject) => setTimeout(() => reject(new Error('Request timeout')), config.timeout)),
                ]);
            }
            catch (error) {
                lastError = error;
                if (attempt < config.maxAttempts) {
                    ...
                    console.warn(`   ⚠️  ${context} failed (attempt ${attempt}/${config.maxAttempts}), ` +
    ...
    async authenticatedRequest(method, endpoint, data) {
        ...
        }, this.DEFAULT_RETRY_CONFIG, `API ${method.toUpperCase()} ${endpoint}`);
    }
    /**
    ...
    normalizeCarePlan(carePlan) {
        try {
            ...
            throw new types_1.NormalizationError(`Failed to normalize CarePlan: ${error.message}`, 'care_plan', carePlan.id, carePlan);
        }
    }
    /**
    ...
    normalizeMedication(medication) {
        try {
            ...
            throw new types_1.NormalizationError(`Failed to normalize Medication: ${error.message}`, 'medication', medication.id, medication);
        }
    }
    /**
     ...
     */
    normalizeClinicalNote(note) {
        try {
            // Extract text from sections using iterative approach
            const textParts = [note.name];
            const stack = [...note.sections];
            while (stack.length > 0) {
                const item = stack.pop();
                if (typeof item === 'string') {
                    textParts.push(item);
            ...
            throw new types_1.NormalizationError(`Failed to normalize ClinicalNote: ${error.message}`, 'note', note.id, note);
        }
    }
    /**
     ...
     */
    async getNormalizedArtifacts(patientId) {
        try {
     ...
     */
    async getNormalizedArtifact(artifactId, type) {
        try {
     ...
     */
    async prefetchPatientData(patientId) {
     ...
     */
    async warmupCache(patientIds) {
        console.log(`🔥 Warming up cache for ${patientIds.length} patients...`);
        const startTime = Date.now();
        const results = await Promise.allSettled(patientIds.map(id => this.prefetchPatientData(id)));
