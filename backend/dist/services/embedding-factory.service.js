...
const ollama_embedding_service_1 = __importDefault(require("./ollama-embedding.service")); // Ollama version
/**
...
class OllamaEmbeddingAdapter {
    async generateEmbedding(text) {
        return ollama_embedding_service_1.default.generateEmbedding(text);
    }
    async generateBatchEmbeddings(texts) {
        return ollama_embedding_service_1.default.generateBatchEmbeddings(texts);
    }
    async getModelInfo() {
        const ollamaConfig = ollama_embedding_service_1.default.getConfig();
        return {
            provider: 'ollama',
    ...
    async healthCheck() {
        return ollama_embedding_service_1.default.healthCheck();
    }
}
/**
...
function getConfiguredProvider() {
    ...
    if (provider && provider !== 'ollama') {
        throw new Error(`SECURITY VIOLATION: EMBEDDING_PROVIDER='${provider}' is not allowed. ` +
    ...
    if (!provider) {
        console.warn('[Embedding Factory] No EMBEDDING_PROVIDER set in environment. Defaulting to "ollama" (HIPAA compliant).');
    }
    return 'ollama';
}
/**
 ...
 * await embeddings.generateEmbedding('text');
 */
function createEmbeddingService(providerType) {
...
async function validateProvider(provider) {
    ...
    if (!isHealthy) {
        ...
        throw new Error(`Ollama embedding provider is not available.\n\n` +
...
exports.default = defaultEmbeddingService;
//# sourceMappingURL=embedding-factory.service.js.map