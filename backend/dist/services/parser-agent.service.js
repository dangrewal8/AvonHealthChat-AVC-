...
const text_chunker_service_1 = __importDefault(require("./text-chunker.service"));
const clinical_ner_service_1 = __importDefault(require("./clinical-ner.service"));
/**
 * Parser Agent Class
 *
...
class ParserAgent {
        ...
        for (const artifact of artifacts) {
            try {
                ...
                for (const chunk of chunks) {
                    const entities = clinical_ner_service_1.default.extractEntities(chunk.chunk_text);
                    // Create parsed chunk
                    const parsedChunk = {
                        ...chunk,
                        entities,
    ...
    parseWithResult(artifacts) {
        ...
        for (const artifact of artifacts) {
            try {
                ...
                this.validateArtifact(artifact);
                // Chunk the artifact
                const artifactChunks = text_chunker_service_1.default.chunk(artifact);
                // Extract entities from each chunk
     ...
     */
    parseInBatches(artifacts, batchSize = this.BATCH_SIZE) {
        const allChunks = [];
        const totalBatches = Math.ceil(artifacts.length / batchSize);
        console.log(`Parser Agent: Processing ${artifacts.length} artifacts in ${totalBatches} batches`);
        for (let i = 0; i < artifacts.length; i += batchSize) {
            const batch = artifacts.slice(i, i + batchSize);
            const batchNumber = Math.floor(i / batchSize) + 1;
            console.log(`Parser Agent: Processing batch ${batchNumber}/${totalBatches} (${batch.length} artifacts)`);
            const batchChunks = this.parse(batch);
            allChunks.push(...batchChunks);
        }
        console.log(`Parser Agent: Completed processing. Total chunks: ${allChunks.length}`);
        return allChunks;
    }
    /**
     ...
     */
    getStatistics(artifacts, result) {
        ...
        return {
            ...
            avg_chunks_per_artifact: successful > 0 ? result.total_chunks / successful : 0,
            avg_entities_per_chunk: result.total_chunks > 0
     ...
     */
    filterByEntityType(chunks, entityType) {
        return chunks.filter((chunk) => chunk.entities.some((entity) => entity.type === entityType));
    }
    /**
     ...
     */
    findChunksWithEntity(chunks, entityText) {
        const searchText = entityText.toLowerCase();
        return chunks.filter((chunk) => chunk.entities.some((entity) => entity.text.toLowerCase() === searchText ||
...
exports.default = parserAgent;
//# sourceMappingURL=parser-agent.service.js.map