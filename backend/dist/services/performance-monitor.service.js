...
class PerformanceMonitor {
     ...
     */
    recordMetric(stage, durationMs, success = true) {
        const record = {
            stage,
    ...
    getMetrics(timeWindowMs) {
        ...
        const endToEndMetrics = recentMetrics.filter(m => m.stage === 'end_to_end');
     ...
     */
    getStageBreakdown(timeWindowMs) {
        const now = Date.now();
        const cutoffTime = timeWindowMs ? now - timeWindowMs : 0;
        // Filter metrics by time window
        const recentMetrics = this.metrics.filter(m => m.timestamp >= cutoffTime);
     ...
     */
    getSlowQueries(thresholdMs, timeWindowMs = 60 * 60 * 1000) {
        const now = Date.now();
        const cutoffTime = now - timeWindowMs;
        return this.metrics
            .filter(m => m.stage === 'end_to_end' &&
     ...
     */
    getCacheHitRate(timeWindowMs = 60 * 60 * 1000) {
     ...
     */
    getErrorRate(timeWindowMs = 60 * 60 * 1000) {
        const now = Date.now();
        const cutoffTime = now - timeWindowMs;
        const recentMetrics = this.metrics.filter(m => m.timestamp >= cutoffTime && m.stage === 'end_to_end');
