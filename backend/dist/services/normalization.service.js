...
class NormalizationService {
    ...
    normalizeLabObservation(rawData) {
        const artifact = {
            id: this.extractId(rawData),
     ...
     */
    normalizeInsurancePolicy(rawData) {
        const artifact = {
            id: this.extractId(rawData),
     ...
     */
    normalizeFamilyHistory(rawData) {
        const artifact = {
            id: this.extractId(rawData),
            type: 'family_history',
            patient_id: this.extractPatientId(rawData),
     ...
     */
    normalizeBatch(rawDataArray, type) {
        return rawDataArray
            .map((rawData) => {
            try {
                const result = this.normalize(rawData, type);
                if (!result.validation.valid) {
                    console.warn(`[Normalization] Artifact ${result.artifact.id} has validation errors:`, result.validation.errors);
                }
     ...
     */
    extractTitle(rawData, fields = ['title', 'name']) {
        for (const field of fields) {
     ...
     */
    extractMedicationTitle(rawData) {
     ...
     */
    extractConditionText(rawData) {
        const parts = [];
        const name = rawData.name || rawData.condition_name || rawData.diagnosis;
        if (name)
            parts.push(`Diagnosis: ${name}`);
        const code = rawData.code || rawData.icd10_code || rawData.icd_code;
        if (code)
     ...
     */
    extractAllergyTitle(rawData) {
        return (rawData.allergen ||
            rawData.substance ||
            rawData.name ||
     ...
     */
    constructSourceUrl(type, id) {
        const baseUrl = env_config_1.default.avon.baseUrl.replace(/\/$/, ''); // Remove trailing slash
        return `${baseUrl}/v2/${type}/${id}`;
    }
    /**
     * Extract metadata (exclude common fields)
     */
    extractMeta(rawData, excludeFields) {
        const meta = {};
    ...
    validateArtifact(artifact) {
        return validation_service_1.default.validate(artifact);
    }
}
exports.normalizationService = new NormalizationService();
exports.default = exports.normalizationService;
