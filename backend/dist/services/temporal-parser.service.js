...
class TemporalParser {
    ...
    parseQuery(query) {
        ...
        if (result.start && result.end) {
            ...
            // Check for "this year" or "this month" patterns
                ...
                dateTo = new Date(parsedDate.getFullYear(), parsedDate.getMonth() + 1, 0, 23, 59, 59, 999);
            }
     ...
     * // }
     */
    parseRelativeTime(query) {
        if (!query || query.trim().length === 0) {
            return null;
        }
        const lowerQuery = query.toLowerCase();
        // Pattern: "last|past X days|weeks|months|years"
        const pastPattern = /\b(?:last|past|previous|in the (?:last|past))\s+(\d+)\s+(day|week|month|year)s?\b/i;
...
exports.default = temporalParser;
//# sourceMappingURL=temporal-parser.service.js.map