...
class CitationLinkGenerator {
     ...
     */
    createLink(provenance, index) {
        // Build deep link URL
        const url = this.buildDeepLink(provenance.artifact_id, provenance.char_offsets, provenance.artifact_type);
        // Format display text
     ...
     */
    buildDeepLink(artifactId, offsets, artifactType = 'note') {
     ...
     * @returns Formatted bibliography string
     */
    formatGroupedBibliography(grouped) {
        let bibliography = 'Sources:\n';
        bibliography += '─'.repeat(80) + '\n\n';
    ...
    generateHTML(links) {
        return links
            .map(link => `<a href="${link.url}" title="${link.tooltip}" class="citation-link">${link.display_text}</a>`)
            .join(' ');
    }
    /**
     ...
     */
    getCitationsByType(links, artifactType) {
     ...
     */
    sortByDate(links, ascending = false) {
...
exports.default = citationLinkGenerator;
//# sourceMappingURL=citation-link-generator.service.js.map