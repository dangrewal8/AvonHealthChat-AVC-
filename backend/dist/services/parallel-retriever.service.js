...
const integrated_retriever_service_1 = __importDefault(require("./integrated-retriever.service"));
...
class ParallelRetriever {
     ...
     */
    async retrieve(query, forceParallel = false) {
     ...
     * @returns Parallel retrieval result
     */
    async retrieveParallel(query, partitions, startTime) {
     ...
     */
    async executePartitionSearch(partition) {
        try {
            const result = await integrated_retriever_service_1.default.retrieve(partition.query);
            return {
                partition,
                result,
            };
     ...
     */
    mergeResults(partitionResults, originalQuery) {
        ...
        const reranked = reranker_service_1.default.rerank(deduplicated, originalQuery, 20);
        const stats = {
     ...
     */
    normalizeScores(candidates, partitionResults) {
        ...
        const normalized = candidates.map((candidate) => {
            ...
            for (const partitionResult of partitionResults) {
                const found = partitionResult.result.candidates.some((c) => c.chunk.chunk_id === candidate.chunk.chunk_id);
                if (found) {
     ...
     */
    aggregateStageMetrics(partitionResults) {
     ...
     */
    async retrieveSequential(query, _startTime) {
     ...
     */
    async comparePerformance(query) {
        // Sequential
    ...
    getSummary(result) {
        ...
        lines.push(``, `Results:`, `  Total Searched: ${result.total_searched}`, `  After Filtering: ${result.filtered_count}`, `  Final Candidates: ${result.candidates.length}`, ``, `Performance:`, `  Average Stage Time: ${Math.round(result.stage_metrics.reduce((sum, s) => sum + s.duration_ms, 0) / result.stage_metrics.length)}ms`, `  Cache Hit: ${result.cache_hit ? 'Yes' : 'No'}`);
        return lines.join('\n');
    }
}
// Export singleton instance
const parallelRetriever = new ParallelRetriever();
exports.default = parallelRetriever;
//# sourceMappingURL=parallel-retriever.service.js.map