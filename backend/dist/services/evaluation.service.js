...
class EvaluationService {
    ...
    submitEvaluation(request) {
        const evaluation = {
            evaluation_id: (0, crypto_1.randomUUID)(),
    ...
    getEvaluationById(evaluationId) {
        return (this.evaluations.find((e) => e.evaluation_id === evaluationId) || null);
    }
    /**
    ...
    calculateStatistics() {
        ...
        const uniqueEvaluators = new Set(this.evaluations.map((e) => e.evaluator)).size;
        ...
        const overallAverage = (criteria.accuracy.mean +
        ...
        const totalIssues = Array.from(issueMap.values()).reduce((sum, count) => sum + count, 0);
    ...
    generateReport() {
        ...
        // Calculate average rating per query
        const queryAverages = Array.from(queryRatings.entries()).map(([query_id, data]) => ({
            query_id,
            query_text: data.query_text,
            average_rating: Math.round((data.ratings.reduce((sum, r) => sum + r, 0) / data.ratings.length) *
                100) / 100,
            evaluation_count: data.count,
        }));
        // Sort by average rating
        const sortedByRating = [...queryAverages].sort((a, b) => a.average_rating - b.average_rating);
        return {
            statistics,
     ...
     */
    calculateCriterionStats(criterion) {
