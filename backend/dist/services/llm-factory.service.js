...
const ollama_llm_service_1 = __importDefault(require("./ollama-llm.service")); // Ollama version
/**
...
class OllamaLLMAdapter {
    async generate(prompt, options) {
        return ollama_llm_service_1.default.generate(prompt, options);
    }
    async generateJSON(prompt, schema, options) {
        return ollama_llm_service_1.default.generateJSON(prompt, schema, options);
    }
    async extract(systemPrompt, userPrompt, temperature = 0) {
        return ollama_llm_service_1.default.extract(systemPrompt, userPrompt, temperature);
    }
    async extractWithRetries(systemPrompt, userPrompt, temperature = 0, maxRetries = 3) {
        return ollama_llm_service_1.default.extractWithRetries(systemPrompt, userPrompt, temperature, maxRetries);
    }
    async summarize(systemPrompt, userPrompt, temperature = 0.3) {
        return ollama_llm_service_1.default.summarize(systemPrompt, userPrompt, temperature);
    }
    async summarizeWithRetries(systemPrompt, userPrompt, temperature = 0.3, maxRetries = 3) {
        return ollama_llm_service_1.default.summarizeWithRetries(systemPrompt, userPrompt, temperature, maxRetries);
    }
    async getModelInfo() {
        const ollamaConfig = ollama_llm_service_1.default.getConfig();
        return {
            provider: 'ollama',
    ...
    async healthCheck() {
        return ollama_llm_service_1.default.healthCheck();
    }
    getTokenUsage() {
        return ollama_llm_service_1.default.getTokenUsage();
    }
    resetTokenUsage() {
        ollama_llm_service_1.default.resetTokenUsage();
    }
}
/**
...
function getConfiguredProvider() {
    ...
    if (provider && provider !== 'ollama') {
        throw new Error(`SECURITY VIOLATION: LLM_PROVIDER='${provider}' is not allowed. ` +
    ...
    if (!provider) {
        console.warn('[LLM Factory] No LLM_PROVIDER set in environment. Defaulting to "ollama" (HIPAA compliant).');
    }
    return 'ollama';
}
/**
 ...
 * await llm.generate('What is diabetes?');
 */
function createLLMService(providerType) {
    ...
    if (provider === 'ollama') {
        ...
        throw new Error(`SECURITY VIOLATION: Provider '${provider}' is not allowed. ` +
            `This system processes Protected Health Information (PHI) and must use ` +
            `local Ollama provider only for HIPAA compliance. ` +
...
async function validateProvider(provider) {
    ...
    if (!isHealthy) {
        ...
        throw new Error(`Ollama LLM provider is not available.\n\n` +
...
exports.default = defaultLLMService;
