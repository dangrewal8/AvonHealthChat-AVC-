...
class ConfidenceScorer {
     ...
     */
    calculateConfidence(retrieval, extractions) {
        // Component 1: Average retrieval score (0-1)
        const avgRetrievalScore = retrieval.candidates.length > 0
        ...
        const supportDensity = this.calculateSupportDensity(extractions, retrieval.candidates);
        // CORRECT FORMULA: 0.6 * retrieval + 0.3 * extraction + 0.1 * support
        const confidenceScore = this.RETRIEVAL_WEIGHT * avgRetrievalScore +
        ...
        return {
            ...
            reason: this.explainConfidence(confidenceScore, avgRetrievalScore, extractionQuality, supportDensity),
        };
    }
    /**
     ...
     */
    assessExtractionQuality(extractions) {
     ...
     */
    calculateSupportDensity(extractions, candidates) {
        if (extractions.length === 0)
            return 0;
        // How many unique sources support the extractions?
     ...
     */
    explainConfidence(score, avgRetrievalScore, extractionQuality, supportDensity) {
     ...
     */
    getConfidenceBreakdown(retrieval, extractions) {
     ...
     */
    identifyWeakestComponent(components) {
        const scores = [
...
exports.default = confidenceScorer;
//# sourceMappingURL=confidence-scorer.service.js.map