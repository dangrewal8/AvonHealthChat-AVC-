...
class TextChunker {
     ...
     */
    splitIntoSentencesWithOffsets(text) {
        const sentences = [];
     ...
     */
    createChunksWithOverlap(sentences, artifact) {
        const chunks = [];
        let i = 0;
        while (i < sentences.length) {
            ...
            const overlapStart = this.findOverlapStart(sentences, i, chunk.sentenceCount, this.OVERLAP_WORDS);
            i = overlapStart;
     ...
     * @returns Chunk
     */
    buildChunk(sentences, startIndex, artifact) {
        let wordCount = 0;
        let sentenceCount = 0;
        ...
        for (let i = startIndex; i < sentences.length; i++) {
            ...
            if (newWordCount <= this.MAX_CHUNK_WORDS ||
     ...
     */
    findOverlapStart(sentences, currentStart, sentenceCount, overlapWords) {
    ...
    getChunkStats(chunks) {
        ...
        return {
            ...
            avg_words_per_chunk: wordCounts.reduce((sum, c) => sum + c, 0) / chunks.length,
            min_words: Math.min(...wordCounts),
            max_words: Math.max(...wordCounts),
            avg_chars_per_chunk: charCounts.reduce((sum, c) => sum + c, 0) / chunks.length,
        };
    }
    /**
     * Verify char offsets
     ...
     */
    verifyOffsets(chunks, originalText) {
...
exports.default = textChunker;
//# sourceMappingURL=text-chunker.service.js.map