...
const indexing_agent_service_1 = __importDefault(require("../services/indexing-agent.service"));
class IndexingController {
     ...
     */
    convertToIndexChunks(textChunks) {
    ...
    async indexPatient(req, res) {
        ...
        try {
            console.log(`[Indexing] Starting indexing for patient ${patientId} (force_reindex: ${force_reindex || false})`);
            // ================================================================
            // STAGE 1: Fetch EMR Data
            // ================================================================
            const fetchStart = Date.now();
            const emrData = await emr_normalized_service_1.default.fetchAll(patientId);
            ...
            console.log(`[Indexing] ✓ Fetched ${totalArtifacts} Tier 1+2+3+4 artifacts (${fetchTime}ms):`, `${notesCount} notes, ${documentsCount} documents, ${medicationsCount} medications, ` +
            ...
            for (const artifact of emrData.artifacts) {
                try {
                    ...
                    console.error(`[Indexing] Failed to chunk artifact ${artifact.id}:`, error instanceof Error ? error.message : error);
                    // Continue with other artifacts even if one fails
                }
            }
            const chunkTime = Date.now() - chunkStart;
            const avgChunksPerArtifact = (textChunks.length / totalArtifacts).toFixed(1);
            console.log(`[Indexing] ✓ Generated ${textChunks.length} chunks from ${totalArtifacts} artifacts (${chunkTime}ms)`, `(avg: ${avgChunksPerArtifact} chunks/artifact)`);
            ...
            const progressCallback = (progress) => {
                console.log(`[Indexing] Stage ${progress.stage}: ${progress.percentComplete.toFixed(0)}% - ` +
                    `Processed ${progress.chunksProcessed}/${progress.chunksTotal} chunks`);
            };
            // Call indexing agent with converted chunks
            const indexingResult = await indexing_agent_service_1.default.indexChunks(indexChunks, progressCallback);
            const indexTime = Date.now() - indexStart;
            console.log(`[Indexing] ✓ Indexed ${indexingResult.chunksIndexed} chunks, ${indexingResult.sentencesIndexed} sentences (${indexTime}ms)`);
            // ================================================================
            ...
            if (indexingResult.errors.length > 0) {
                response.errors = indexingResult.errors;
            }
...
exports.indexingController = new IndexingController();
exports.default = exports.indexingController;
